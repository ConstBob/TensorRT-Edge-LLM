...
private:
  ...
  static UnderlyingType constexpr kFinished{kFinishedEos | kFinishedStopWords |
                                            kFinishedMaxLength};
...
public:
  ...
  std::optional<std::vector<uint64_t>>
      randomSeed; // [1] or [setupBatchSize] on cpu
...
public:
  ...
  std::optional<std::vector<std::int32_t>>
      runtimeTopK; // [1] or [setupBatchSize] on cpu
  std::optional<std::vector<float>>
      runtimeTopP; // [1] or [setupBatchSize] on cpu

  // topPSamplingLayer
  std::optional<std::vector<float>>
      topPDecay; // [setupBatchSize], must between [0, 1]
  std::optional<std::vector<float>>
      topPMin; // [setupBatchSize], must between [0, 1]
...
public:
  ...
  [[nodiscard]] __host__ __device__ static constexpr float
  ...
  [[nodiscard]] __host__ __device__ static constexpr std::int32_t
  getMinLength() {
    return 1;
  }
