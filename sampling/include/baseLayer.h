...
public:
  ...
  [[nodiscard]] cudaStream_t getStream() const noexcept { return nullptr; }

  //! @returns workspace needed for this layer in bytes
  [[nodiscard]] virtual size_t getWorkspaceSize() const noexcept { return 0; };
  ...
  // clang-format on
  virtual void setup(std::int32_t batchSize, std::int32_t beamWidth,
                     BufferConstPtr batchSlots,
                     std::shared_ptr<BaseSetupParams> const &setupParams) = 0;
  ...
  // clang-format on
  virtual void
  forwardAsync(std::shared_ptr<BaseDecodingOutputs> const &outputs,
               std::shared_ptr<BaseDecodingInputs> const &inputs) = 0;
