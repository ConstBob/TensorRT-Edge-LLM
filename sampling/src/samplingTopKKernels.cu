...
__global__ void
topKStage1(T const *__restrict logProbs,
           T const *const *__restrict logProbsPtrs, T *tmpLogProbs,
           std::int32_t *topKTmpIdBuf, T *topKTmpValBuf,
           FinishedState const *finished, std::int32_t maxTopK,
           std::int32_t const *topKs, std::int32_t vocabSize,
           std::int32_t const *endIds, bool const *skipDecode,
           std::int32_t const *batchSlots, std::int32_t const *tokensPerStep,
           std::int32_t maxTokensPerStep) {
  typedef cub::BlockReduce<TopK_2<T>, BLOCK_SIZE_> BlockReduce;
  __shared__ typename BlockReduce::TempStorage tempStorage;

  auto const tid = static_cast<std::int32_t>(threadIdx.x);
  auto const bid = static_cast<std::int32_t>(blockIdx.x);
  auto const tokenIdx = static_cast<std::int32_t>(blockIdx.y);
  ...
  FinishedState const finishState =
      finished != nullptr ? finished[batchSlot] : FinishedState::empty();
  if ((skipDecode != nullptr && skipDecode[batchSlot]) ||
      (finishState.isSkipDecoding())) {
    return;
  }

  auto const logBufIndex =
      batchId * maxTokensPerStep * vocabSize + tokenIdx * vocabSize;
  auto logProbsSlot = logProbsPtrs == nullptr
                          ? logProbs + logBufIndex
                          : logProbsPtrs[batchId * maxTokensPerStep + tokenIdx];

  auto const blockLane = bid % BLOCKS_PER_BEAM_; // block id for a beam
  auto const k =
      (topKs != nullptr) ? topKs[batchSlot] : maxTopK; // batchId = batch index

  auto const tmpLogBufIndex =
      batchId * maxTokensPerStep * vocabSize + tokenIdx * vocabSize;
  auto const tmpTopKBufIndex =
      batchId * maxTokensPerStep * BLOCKS_PER_BEAM_ * maxTopK +
  ...
  for (std::int32_t ite = 0; ite < k; ite++) {
    partial.init();
#pragma unroll
    for (auto elemId = tid + blockLane * BLOCK_SIZE_; elemId < vocabSize;
         elemId += BLOCK_SIZE_ * BLOCKS_PER_BEAM_) {
    ...
    TopK_2<T> total =
        BlockReduce(tempStorage).Reduce(partial, reduce_topk_op_2<T>);
...
__global__ void topKStage2Sampling(
    ...
    curandState_t *curandState, std::int32_t const *endIds,
    std::int32_t vocabSize, bool const *skipDecode,
    std::int32_t const *batchSlots, std::int32_t maxBatchSize,
    bool normalizeLogProbs, bool logitHasProbs,
    std::int32_t const *tokensPerStep, std::int32_t maxTokensPerStep,
    std::int32_t maxSeqLen, bool returnAllTopK) {
  bool const IS_FP16 = std::is_same<T, half>::value;
  T const MAX_T_VAL = (IS_FP16) ? HALF_FLT_MAX : FLT_MAX;

  auto const tid = static_cast<std::int32_t>(threadIdx.x);
  auto const batchIdx = static_cast<std::int32_t>(blockIdx.x);
  auto const tokenIdx = static_cast<std::int32_t>(blockIdx.y);
  auto const batchSlot =
      batchSlots != nullptr ? batchSlots[batchIdx] : batchIdx;
  FinishedState const finishState = finishedInput != nullptr
                                        ? finishedInput[batchSlot]
                                        : FinishedState::empty();
  if ((skipDecode != nullptr && skipDecode[batchSlot]) ||
      (finishState.isSkipDecoding())) {
    return;
  ...
  auto *sId = reinterpret_cast<std::int32_t *>(array);
  if (tid == 0) {
    sSum = 0.0f;
  }
  ...
  for (std::int32_t ite = 0; ite < k; ite++) {
    partial.init();
#pragma unroll
    for (std::int32_t i = tid; i < size; i += BLOCK_SIZE_) {
      partial.insert((float)sVal[i], i);
    }

    TopK_2<float> total =
        BlockReduce(tempStorage).Reduce(partial, reduce_topk_op_2<float>);
  ...
  if (tid == 0) {
    auto randNum = static_cast<float>(curand_uniform(curandState + batchSlot) *
                                      probThreshold * sSum);
    auto *outputIdsRequestPtr =
        idsPtrs == nullptr ? ids + batchSlot * maxSeqLen : idsPtrs[batchSlot];
    for (std::int32_t ki = 0; ki < k; ki++) {
      ...
      if (randNum <= 0.0f || ki == k - 1 || returnAllTopK) {
        ...
        // to get vivid indicator of smth going wrong for the debug
        auto outputId =
            idx != -1 ? topKTmpIdBuf[(batchIdx * maxTokensPerStep + tokenIdx) *
                                         stride +
                                     idx] %
                            vocabSize
                      : vocabSize - 1;
        auto const curSeqLen =
            sequenceLengths == nullptr ? 0 : sequenceLengths[batchSlot];
        auto const outIdx =
            returnAllTopK ? tokenIdx * maxTopK + ki : curSeqLen + tokenIdx;
    ...
    if (maxTokensPerStep == 1 && !returnAllTopK && sequenceLengths != nullptr &&
  ...
  do {                                                                         \
    {                                                                          \
      ...
      topKStage1<T, BLOCK_SIZE_1_, BLOCKS_PER_BEAM_>                           \
          <<<grid, block, 0, stream>>>(                                        \
              params.logProbs, params.logProbsPtrs, tempLogProbs,              \
              topKTmpIdBuf, topKTmpValBuf, params.finishedInput,               \
              params.maxTopK, params.topKs, params.vocabSizePadded,            \
              params.endIds, params.skipDecode, params.batchSlots,             \
    ...
    {                                                                          \
      ...
      topKStage2Sampling<T, BLOCK_SIZE_2_, BLOCKS_PER_BEAM_>                   \
          <<<grid, block,                                                      \
             K_MAX * sizeof(std::int32_t) + K_MAX * sizeof(float), stream>>>(  \
              topKTmpIdBuf, topKTmpValBuf, params.outputIdsPtrs,               \
              params.outputIds, params.sequenceLengths, params.finishedInput,  \
              params.finishedOutput, params.cumLogProbs,                       \
              params.outputLogProbs, params.maxTopK, params.topKs,             \
              params.maxTopP, params.topPs, params.curandState, params.endIds, \
              params.vocabSizePadded, params.skipDecode, params.batchSlots,    \
              params.maxBatchSize, params.normalizeLogProbs,                   \
              params.logitsHasProbs, params.tokensPerStep,                     \
              params.maxTokensPerStep, params.maxSeqLen,                       \
              params.returnAllTopK);                                           \
    }                                                                          \
  } while (0)

template <typename T>
void invokeBatchTopKSampling(TopKSamplingKernelParams<T> const &params,
                             cudaStream_t stream) {
  params.checkParams();

  // Not allow an ambiguous inputs topP and topPs.
  auto const workspaceSizes =
      getTopKWorkspaceSizes<T>(params.batchSize, params.maxTokensPerStep,
                               params.maxTopK, params.vocabSizePadded);
  ...
  auto topKTmpIdBuf = static_cast<std::int32_t *>(alignedPointers[1]);
  auto topKTmpValBuf = static_cast<T *>(alignedPointers[2]);

  std::int32_t logMaxTopK{0};
  std::int32_t recursor{params.maxTopK - 1};
  while (recursor >>= 1) {
    ++logMaxTopK;
  }
...
template void
invokeBatchTopKSampling(TopKSamplingKernelParams<float> const &params,
                        cudaStream_t stream);

template void
invokeBatchTopKSampling(TopKSamplingKernelParams<half> const &params,
                        cudaStream_t stream);
