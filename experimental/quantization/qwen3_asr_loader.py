...
:func:`experimental.quantization.quantize._load_model` raise
``ValueError: Unrecognized configuration class``. This module wraps the
standalone joint calibration model in
:mod:`experimental.quantization.models.qwen3_asr` with the bookkeeping
...
:mod:`experimental.quantization.models.qwen3_asr` -- pure ``nn.Linear``
throughout, no imports from :mod:`experimental.llm_loader`. The runtime
side has its own quant-aware modeling tree under
:mod:`experimental.llm_loader.models.qwen3_asr` for ONNX export.
"""
...
# Default config knobs for the LibriSpeech calibration loader. Mirror the
# legacy ``tensorrt_edgellm.quantization.audio_quantization`` recipe so
# ``--dataset openslr/librispeech_asr`` Just Works for ASR calibration.
...
def load_qwen3_asr_joint_for_calibration(model_dir: str, torch_dtype, device):
    ...
    contract of :func:`experimental.quantization.quantize._load_model`.
...
def asr_calibration_dataloader(
    ...
    :func:`experimental.quantization.quantize._calibrate_asr_multimodal`).
...
def postprocess_qwen3_asr_checkpoint(model_dir: str, output_dir: str) -> None:
    ...
    ``llm_loader.config._normalize_module_name`` strips the ``model.``
