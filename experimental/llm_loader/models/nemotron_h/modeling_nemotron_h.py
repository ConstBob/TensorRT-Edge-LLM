...
class RMSNorm(nn.Module):
    """RMSNorm for hybrid Mamba models — exports as a single ONNX op.

    Uses ``torch.nn.functional.rms_norm`` so the dynamo exporter emits a
    single ``RMSNormalization`` node that TRT handles natively instead of
    decomposed Mul+ReduceMean+Rsqrt.  The decomposed pattern triggers
    TRT's "Force Reduce to FP32" heuristic, which creates FP32
    intermediates incompatible with Mamba plugin FP16 requirements
    (``ForeignNode`` tactic failure).
    """
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        return F.rms_norm(hidden_states, (self.hidden_size, ), self.weight,
...
def _make_flat_wrapper_mamba(model: nn.Module, Na: int, Nm: int) -> nn.Module:
    ...
    inputs and ``present_conv_state_i`` / ``present_ssm_state_i`` outputs.
    ...
    ssm_tuple = "({},)".format(", ".join(f"ssm_state_{i}"
                                         for i in range(Nm))) if Nm else "()"
...
class NemotronHCausalLM(nn.Module):
    ...
    def onnx_export_spec(self) -> OnnxSpec:
        ...
        output_names = (["logits"] +
                        ...
                        [f"present_ssm_state_{i}" for i in range(Nm)])
        ...
        for _ in range(Nm):
            all_shapes.append({0: batch})  # ssm_state_i
