...
class Qwen3SparseMoeBlock(nn.Module):
    ...
    def __init__(self, config: ModelConfig) -> None:
        ...
        # ``NVFP4_MOE_BACKEND_*`` constants in ``llm_loader/config.py``.
        ...
        # GeForce-only attributes; values track the legacy defaults that
        # ``NvFP4MoEPluginGeforce`` consumed before this switch was added.
    ...
    def _prepare_nvfp4_moe_weights_thor(self) -> None:
        ...
        # Slots 9 / 10 are legacy (decode now reads the prefill SF at slots
        # 4 / 7); the plugin still validates them as INT8 tensors.
