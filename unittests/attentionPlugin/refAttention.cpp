...
#define INSTANTIATE_refAttention(prec)                                                                                 \
    template Eigen::Matrix<float, headGrpSize, validElemsPerHead, Eigen::RowMajor> refAttention<prec>(                 \
        IOHead const* q, CacheSeq const& k, CacheSeq const& v, uint32_t seqLen, float kvScale, float xScale)
...
#define INSTANTIATE_refFlashAttention(prec, tileSize)                                                                  \
    template Eigen::Matrix<float, headGrpSize, validElemsPerHead, Eigen::RowMajor> refFlashAttention<prec, tileSize>(  \
        IOHead const* q, CacheSeq const& k, CacheSeq const& v, uint32_t seqLen, float kvScale, float xScale)
