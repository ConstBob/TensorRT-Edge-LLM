...
TEST(ParallelConfigTest, UsesDefaultShmAllReducePolicy)
{
    // SHM is an optional acceleration path; NCCL remains the required baseline and fallback.
    ParallelConfig const config{};

    EXPECT_TRUE(config.shmAllReduceConfig.enableShmAllReduce);
    EXPECT_EQ(config.shmAllReduceConfig.shmMaxElements, 0);
    EXPECT_EQ(config.shmAllReduceConfig.shmAllReduceElementThreshold, 0);
    EXPECT_EQ(config.shmAllReduceConfig.shmFp8SmallPathElementThreshold, 8192);
    EXPECT_NE(config.toString().find("enableShmAllReduce=true"), std::string::npos);
    EXPECT_NE(config.toString().find("shmFp8SmallPathElementThreshold=8192"), std::string::npos);
}
