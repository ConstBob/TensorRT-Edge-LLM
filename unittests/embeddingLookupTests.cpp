...
TEST_F(EmbeddingLookupTest, StandardEmbeddingLookupAccuracy)
{
    ...
    for (auto const& [batchSize, seqLen, vocabSize, hiddenSize] : testCases)
    {
        ...
        // Run GPU kernel
        kernels::embeddingLookup(inputIdsTensor, embeddingTableTensor, outputTensor);
...
TEST_F(EmbeddingLookupTest, UnevenHiddenSizeError)
{
    ...
    EXPECT_THROW(
        { kernels::embeddingLookup(inputIdsTensor, embeddingTableTensor, outputTensor); }, std::runtime_error)
