...
class GemmBlackwellNvFp4WS:
    ...
    def wrapper(
        ...
        # design wrote ONE row (m=1 at decode) of FP8 directly to the
        # slot — 2.5 KB system-coherent store, fast. With a
        # (m_padded, n) C layout, the kernel would write ALL 128 rows
        # = 327 KB (for 4B o_proj at decode) to the slot — 100× slower
        # because each row goes over the host-coherent fabric. To
        # accommodate that, the runner would need an M-padding GPU-DRAM
