...
activations are the B/N operand, so the ragged token axis of a MoE GEMM is the
MMA N mode.

What is added for grouping:

* Weights and block scales carry the expert as their L mode.  Both TMA
  descriptors are built with L = num_experts (one base pointer, one expert
  plane per L step), so ``tile_group_idx[n_tile]`` selects the expert of a
  token tile by choosing the L coordinate -- no tensormap updates.
* Activations are read from a permuted, expert-contiguous ``[R_pad, K]``
  buffer whose per-expert segments are padded to whole token tiles.
* Every scheduler-owning warp skips token tiles ``>= num_valid_tiles[0]`` (a
  device value written by ``buildLayoutGpu``), so the host launches a
  CUDA-graph-stable conservative grid.
* Per-expert ``global_scale[expert]`` (verbatim fp32 ``weight_scale_2``) is
  applied in the fp32 epilogue.  Fusions:
    - ``relu2_store``: ``relu(alpha*acc)^2`` -> FP16/BF16, TMA store to the
      permuted ``[R_pad, N]`` intermediate (FC1).
    - ``scatter_add``: ``alpha * topk_weight[token, slot] * acc`` scatter-added
      with ``red.global.v4.{f16,bf16}x2`` into ``out[token, :]`` through
...
@dsl_user_op
def red_add_v4_halfx2(words: cute.Tensor,
                      dst_ptr,
                      is_bf16: bool,
                      *,
                      loc=None,
                      ip=None):
    """``red.global.v4.{f16x2|bf16x2}.add.noftz [dst], {w0..w3}`` (16 bytes, 8 halves)."""
    op = "bf16x2" if is_bf16 else "f16x2"
    llvm.inline_asm(
        None,
        [
            dst_ptr.llvm_ptr,
            words[0].ir_value(loc=loc, ip=ip),
            words[1].ir_value(loc=loc, ip=ip),
            words[2].ir_value(loc=loc, ip=ip),
            words[3].ir_value(loc=loc, ip=ip),
        ],
        f"red.global.v4.{op}.add.noftz [$0], {{$1, $2, $3, $4}};",
        "l,r,r,r,r",
        has_side_effects=True,
        is_align_stack=False,
        asm_dialect=llvm.AsmDialect.AD_ATT,
...
FUSION_RELU2_STORE = "relu2_store"
FUSION_SCATTER_ADD = "scatter_add"
#: Upper bound on token tiles per launch; ``tile_group_idx`` is staged into
#: shared memory once per CTA so no warp pays a global round trip per tile.
#: The runner enforces ``num_rows_padded / token_tile <= MAX_TOKEN_TILES``.
MAX_TOKEN_TILES = 1024
_FUSIONS = (FUSION_RELU2_STORE, FUSION_SCATTER_ADD)


class Nvfp4A16BlackwellMoeGemm:
    """
    Grouped mixed-input GEMM kernel for NVIDIA Blackwell SM110 architecture.
    ...
    def __init__(
        ...
        ``ctas_per_sm`` splits the SMEM and TMEM budgets so that ``ctas_per_sm``
        persistent CTAs can be resident per SM (experiment knob; 1 in production,
        see Nvfp4A16BlackwellMoeGemmLaunch).
        """
        if fusion not in _FUSIONS:
            raise ValueError(
                f"fusion must be one of {_FUSIONS}, got {fusion!r}")
        if ctas_per_sm not in (1, 2):
            raise ValueError("ctas_per_sm must be 1 or 2")
        self.ctas_per_sm = ctas_per_sm
        if not use_tma_store:
            raise ValueError(
                "the grouped kernel stages C through SMEM (use_tma_store=True)"
            )
        self.fusion = fusion
        # relu2_store TMA-stores C; scatter_add only stages C in SMEM (fp32) and
        # scatter-adds it into the token output.
        self.use_tma_c_store = fusion == FUSION_RELU2_STORE
        ...
        # Set transform mode
        if cutlass.const_expr(self.scale_granularity_m == 0
                              and self.scale_granularity_k == 0):
        ...
        if cutlass.const_expr(self.scale_mode == TransformMode.ConvertScale):
            self.scale_tile_shape = (
                (cute.size(self.mma_tiler[0]) //
                 2 if self.use_2cta_instrs else cute.size(self.mma_tiler[0])),
            ...
            scale_words_outer = cute.make_layout((scale_rows, 1),
                                                 stride=(1, 0))
    ...
    def _validate_inputs(
        ...
        if cutlass.const_expr(
                a_scale is None
                or a_scale.element_type is not cutlass.Float8E4M3FN):
            raise ValueError("block scales must use raw Float8E4M3FN elements")
        if cutlass.const_expr(b.element_type not in (cutlass.Float16,
                                                     cutlass.BFloat16)):
            raise ValueError("activation must be FP16 or BF16")
        if cutlass.const_expr(self.fusion == FUSION_RELU2_STORE):
            if cutlass.const_expr(c.element_type is not b.element_type):
                raise ValueError(
                    "relu2_store output must match the activation dtype")
        else:
            if cutlass.const_expr(c.element_type is not cutlass.Float32):
                raise ValueError("scatter_add stages the accumulator as fp32")
        if cutlass.const_expr(self.scale_granularity_m != 1
                              or self.scale_granularity_k != 16
                              or self.mma_tiler[2] != 64):
            raise ValueError(
                "Blackwell opaque layout requires M1/K16 scales and MMA K64")
    ...
    def __call__(
        ...
        tile_group_idx: cute.Tensor,
        num_valid_tiles: cute.Tensor,
        ...
        """
        Executes the grouped Mixed Input GEMM operation.

        :param global_scale: ``[num_experts]`` fp32 per-expert weight_scale_2.
        :param tile_group_idx: ``[num_token_tiles]`` int32 expert of each token tile.
        :param num_valid_tiles: ``[1]`` int32 device count of non-empty token tiles.
        :param permuted_idx: ``[R_pad]`` int32 ``token*topK+slot`` per permuted row
        ...
        a_op = mixed_input_utils.get_tma_atom_kind(self.is_a_mcast,
                                                   self.use_2cta_instrs, False)
        b_op = mixed_input_utils.get_tma_atom_kind(self.is_b_mcast,
                                                   self.use_2cta_instrs, True)
        a_scale_op = a_op
        # Deduce TMA copy atom and TMA tensor for A, scale, and B
        smem_layout_a_per_stage = cute.slice_(self.smem_layout_a,
                                              (None, None, None, 0))
            ...
            scale_words = cute.make_tensor(
                ...
                cute.make_layout(
                    (
                        ...
                        scale_k_tiles,
                        # L = num_experts: the expert plane is selected by the
                        # scale TMA warp through its L coordinate.
                        cute.size(a_scale.layout.shape[2]),
                    ),
                    stride=(
                        (1, scale_k_tiles * self.scale_tile_shape[0]),
                        self.scale_tile_shape[0],
                        scale_n_tiles * scale_k_tiles *
                        self.scale_tile_shape[0],
                    ),
                ),
            )
        ...
        smem_layout_b_per_stage = cute.slice_(self.smem_layout_b,
                                              (None, None, None, 0))
        ...
        self.num_tma_load_bytes_b = b_copy_size * cute.size(
            tiled_mma.thr_id.shape)
        self.num_tma_load_bytes_scale = a_scale_copy_size
        # max_active_clusters arrives as the SM count; the persistent grid
        # holds ctas_per_sm CTAs per SM (extra CTAs simply queue if the device
        # cannot co-schedule them, so this is never a correctness concern).
        self.tile_sched_params, grid = self._compute_grid(
            c,
            self.cta_tile_shape_mnk,
            self.cluster_shape_mn,
            max_active_clusters * self.ctas_per_sm,
        )
        ...
        if cutlass.const_expr(self.use_tma_c_store):
        ...
        a_transform_smem_size = (cute.cosize(
            self.smem_layout_a_transform.outer) if self.transform_a_source
                                 == tcgen05.OperandSource.SMEM else 0)
        a_scale_smem_size = (cute.cosize(self.smem_layout_scale.outer)
                             if self.scale_mode is TransformMode.ConvertScale
                             else 0)
        ...
        class SharedStorage:
            ...
            # Expert of each token tile (tile_group_idx staged once per CTA).
            tile_expert: cute.struct.MemRange[cutlass.Int32, MAX_TOKEN_TILES]
            # Tensor buffers
        ...
        self.kernel(
            ...
            tma_tensor_b,
            tma_atom_c,
            tma_tensor_c if self.use_tma_c_store else c,
            global_scale,
            tile_group_idx,
            num_valid_tiles,
            permuted_idx,
    ...
    def kernel(
        ...
        mC_mnl: cute.Tensor,
        global_scale: cute.Tensor,
        mTileExpert: cute.Tensor,
        mNumValidTiles: cute.Tensor,
        mPermutedIdx: cute.Tensor,
        ...
        transform_thread_idx = (tidx -
                                32 * self.transform_warp_id[0] if tidx >= 32 *
                                self.transform_warp_id[0] else tidx)
        a_load2trans_pipeline = pipeline.PipelineTmaAsync.create(
            ...
            mcast_mode_mn=(
                1, 0),  # multicast for A will only happen on the M-mode
            defer_sync=True,
        )
        ...
        if cutlass.const_expr(self.scale_mode == TransformMode.ConvertScale):
            ...
            scale_load2trans_pipeline = pipeline.PipelineTmaAsync.create(
                barrier_storage=storage.a_scale_load2trans_full_mbar_ptr.
                data_ptr(),
                ...
                consumer_group=pipeline.CooperativeGroup(
                    pipeline.Agent.Thread,
                    num_producers_a_scale * len(self.transform_warp_id) *
        ...
        b_load2mma_pipeline = pipeline.PipelineTmaUmma.create(
            ...
            cta_layout_vmnk=cluster_layout_vmnk,
            mcast_mode_mn=(
                0, 1),  # multicast for B will only happen on the N-mode
            defer_sync=True,
        )
        ...
        sC = (storage.smem_C.get_tensor(c_smem_layout_staged.outer,
                                        swizzle=c_smem_layout_staged.inner)
              if self.use_tma_store else None)
        sA_input = storage.smem_A.get_tensor(a_smem_layout.outer,
                                             swizzle=a_smem_layout.inner)
        sS_transform = (storage.smem_A_scale.get_tensor(
            scale_smem_layout.outer, swizzle=scale_smem_layout.inner)
                        if self.scale_mode is TransformMode.ConvertScale else
                        None)
        sS_words = (cute.make_tensor(
        ...
        if cutlass.const_expr(
            ...
            tmem_ptr_transform = cute.recast_ptr(accumulators.iterator +
                                                 self.num_acc_tmem_cols,
                                                 dtype=self.mma_dtype)
            tCrA = cute.make_tensor(
                tmem_ptr_transform,
                tiled_mma.make_fragment_A(
                    a_smem_layout_transform.outer).layout,
            )
        else:
        ...
        if warp_idx == self.tma_warp_id:
            ...
            while work_tile.is_valid_tile:
                ...
                tile_is_active = cur_tile_coord[1] < num_valid_tiles_value
                if tile_is_active:
                    mma_tile_coord_mnl = (
                    ...
                    for k_tile in cutlass.range(0, k_tile_cnt, 1, unroll=1):
                        ...
                        cute.copy(
                            tma_atom_b,
                            tBgB_slice[(None,
                                        b_load2mma_producer_state.count)],
                            tBsB[(None, b_load2mma_producer_state.index)],
                            tma_bar_ptr=b_load2mma_pipeline.
                            producer_get_barrier(b_load2mma_producer_state),
                            mcast_mask=b_full_mcast_mask,
                        )
                        a_load2trans_pipeline.producer_commit(
                            a_load2trans_producer_state)
                        b_load2mma_pipeline.producer_commit(
                            b_load2mma_producer_state)
        ...
        if warp_idx == self.scale_tma_warp_id:
            ...
            if cutlass.const_expr(
                ...
                # Persistent tile scheduling loop
                tile_sched = utils.StaticPersistentTileScheduler.create(
                    tile_sched_params, (bidx, bidy, bidz),
                    cute.arch.grid_dim())
                work_tile = tile_sched.initial_work_tile_info()
                ...
                while work_tile.is_valid_tile:
                    ...
                    if tile_is_active:
                        ...
                        # (atom_v, RestK) of the expert owning this token tile
                        expert_idx = cutlass.Int32(
                            sTileExpert[cur_tile_coord[1]])
                        tSgS_slice = tSgS[(None, mma_tile_coord_mnl[0], None,
                                           expert_idx)]
                        ...
                        for k_tile in cutlass.range(0,
                            ...
                            cute.copy(
                                ...
                                tSgS_slice[(
                                    None,
                                    scale_load2trans_producer_state.count)],
                                tSsS[(None,
                                      scale_load2trans_producer_state.index)],
                                tma_bar_ptr=scale_load2trans_pipeline.
                            ...
                            peek_scale_load2trans_empty_status = cutlass.Boolean(
                                1)
                            if scale_load2trans_producer_state.count < scale_k_tile_cnt:
                                peek_scale_load2trans_empty_status = (
                                    scale_load2trans_pipeline.
                ...
                scale_load2trans_pipeline.producer_tail(
                    scale_load2trans_producer_state)
        ...
        if warp_idx >= self.transform_warp_id[0]:
            ...
            (
                ...
                sA_input,
                (tCrA if self.transform_a_source == tcgen05.OperandSource.TMEM
                 else sA_transform),
            ...
            if cutlass.const_expr(
                ...
                smem_thr_copy_S, tSsS_trans, tSrS_copy, tSrS = (
                    mixed_input_utils.scale_partition(src_copy_a, tCsS,
                                                      transform_local_tidx,
                                                      self.a_scale_dtype))
                assert cute.size(tSrS, mode=[0]) == cute.size(tArA, mode=[
                    0
            ...
            tArA_transform_store = cute.flat_divide(tArA_transform,
                                                    transform_tiler)
                    ...
                    for k_tile in cutlass.range(0, k_tile_cnt, 1, unroll=1):
                        ...
                        if cutlass.const_expr(
                            ...
                            if k_tile % num_k_tiles_per_scale == 0:
                                ...
                                cute.autovec_copy(tSsS_slice_filtered,
                                                  tSrS_copy)
                        ...
                        for idx in cutlass.range_constexpr(
                            ...
                            if cutlass.const_expr(
                                ...
                                packed_source = cute.recast_tensor(
                                    source_fragment, Uint8)
                            ...
                            if cutlass.const_expr(self.scale_mode ==
                                                  TransformMode.ConvertScale):
                                ...
                                for element_idx in cutlass.range_constexpr(
                                    ...
                                    expanded_scale[element_idx] = tSrS_load[
                                        cute.idx2crd(element_idx,
                                                     tSrS_load.shape)].to(
                                                         self.mma_dtype)
                                # The per-expert global scale is applied in the fp32
                                # epilogue; only the E4M3 block scale is folded here.
                                tensor_transformed = tensor_transformed * expanded_scale.load(
                                )
                            tArA_transform_store[(
                                None, idx)].store(tensor_transformed)
                        # Store transformed A to tensor memory or shared memory
                        mixed_input_utils.store_transformed_a(
                            tArA_transform,
                            tAsA_transform[(None, None, None, None,
                        ...
                        if cutlass.const_expr(self.transform_a_source ==
                        ...
                        a_load2trans_pipeline.consumer_release(
                            cur_a_load2trans_consumer_state)
                        # Signal the completion of transformation
                        trans2mma_pipeline.producer_commit(
                            trans2mma_producer_state)
                    ...
                    if is_leader_cta:
                        ...
                        for k_tile in cutlass.range(0, k_tile_cnt, 1,
                            ...
                            trans2mma_pipeline.consumer_wait(
                                trans2mma_consumer_state,
                                peek_trans2mma_full_status)
                            b_load2mma_pipeline.consumer_wait(
            ...
            while work_tile.is_valid_tile:
                ...
                if tile_is_active:
                    ...
                    if cutlass.const_expr(self.use_tma_c_store):
                        ...
                        tTR_gC = tTR_gC_partitioned[(None, None, None, None,
                                                     None,
                                                     *mma_tile_coord_mnl)]
                    ...
                    # Per-expert fp32 global scale (verbatim weight_scale_2) of the
                    # expert owning this token tile.
                    expert_idx = cutlass.Int32(sTileExpert[cur_tile_coord[1]])
                    alpha_val = cutlass.Float32(global_scale[expert_idx])
                    # Store accumulator to global memory in subtiles
                    subtile_cnt = cute.size(tTR_tAcc.shape, mode=[3])
                    num_prev_subtiles = tile_sched.num_tiles_executed * subtile_cnt
                    tCompute = cute.make_rmem_tensor(tTR_rAcc.shape,
                                                     self.acc_dtype)
                    for subtile_idx in cutlass.range(subtile_cnt):
                        ...
                        if cutlass.const_expr(
                                self.fusion == FUSION_RELU2_STORE):
                            # FC1: relu(alpha * acc)^2 in fp32, narrowed once.
                            for i in cutlass.range_constexpr(
                                    cute.size(tTR_rAcc.shape)):
                                scaled = tTR_rAcc[i] * alpha_val
                                relu_val = cute.arch.fmax(
                                    scaled, cutlass.Float32(0.0))
                                tCompute[i] = relu_val * relu_val
                            acc_vec = tiled_copy_r2s.retile(tCompute).load()
                        else:
                            # FC2: stage the raw fp32 accumulator; alpha and the
                            # router weight are applied per row at scatter time.
    ...
    def scatter_add_subtile(
        ...
        """Scatter-add one staged fp32 epilogue subtile into ``out[token, :]``.

        The subtile sits in SMEM as ``(EPI_M, EPI_N)`` with M (output features)
        contiguous.  Each work item is one permuted row (token) and one chunk of
        8 output features; ``red.global.v4.{f16,bf16}x2.add`` writes 16 bytes.
        Consecutive threads take consecutive chunks of the same row so a warp
        covers whole 256-byte row segments.  Rows whose ``permuted_idx`` is
        negative are padding and are skipped.
        """
        epi_shape = cute.product_each(cute.shape(epi_tile))
        epi_m = cute.size(epi_shape[0])
        epi_n = cute.size(epi_shape[1])
        num_em = self.cta_tile_shape_mnk[0] // epi_m
        chunks_per_row = epi_m // 8
        num_items = epi_n * chunks_per_row
        num_threads = 32 * len(self.epilog_warp_id)
        em = subtile_idx % num_em
        en = subtile_idx // num_em
        m_base = mma_tile_coord_mnl[0] * self.cta_tile_shape_mnk[0] + em * epi_m
        row_base = mma_tile_coord_mnl[1] * self.cta_tile_shape_mnk[
            1] + en * epi_n
        top_k = cute.size(mTopkWeights.shape[1])
        is_bf16 = self.out_dtype is cutlass.BFloat16

        sC_buf = sC[(None, None, c_buffer)]
        # (8, EPI_M/8, EPI_N): 16-byte chunks along the contiguous M mode
        sC_chunks = cute.flat_divide(sC_buf, (8, ))
        frag = cute.make_rmem_tensor((8, ), self.c_dtype)
        frag_out = cute.make_rmem_tensor((8, ), self.out_dtype)
        words = cute.recast_tensor(frag_out, cutlass.Uint32)
        for it in cutlass.range_constexpr(cute.ceil_div(
                num_items, num_threads)):
            item = it * num_threads + epi_tidx
            if item < num_items:
                chunk = item % chunks_per_row
                row = item // chunks_per_row
                expanded_idx = mPermutedIdx[row_base + row]
                if expanded_idx >= 0:
                    token_idx = expanded_idx // top_k
                    slot_idx = expanded_idx - token_idx * top_k
                    row_scale = alpha_val * mTopkWeights[(token_idx, slot_idx)]
                    cute.autovec_copy(sC_chunks[(None, chunk, row)], frag)
                    for i in cutlass.range_constexpr(8):
                        frag_out[i] = (frag[i] * row_scale).to(self.out_dtype)
                    dst = cute.domain_offset((token_idx, m_base + chunk * 8),
                                             mOut)
                    red_add_v4_halfx2(words, dst.iterator, is_bf16)
        ...
        if self.use_tma_store:
            tma_atom_c, bSG_sC, bSG_gC, _, _ = (
                mixed_input_utils.epilog_gmem_copy_and_partition(
                    self.c_dtype, tidx, atom, None, gC_mnl, None, epi_tile,
                    sC))
            return tma_atom_c, bSG_sC, bSG_gC
        else:
            _, _, _, simt_atom, tTR_gC = (
                mixed_input_utils.epilog_gmem_copy_and_partition(
                    self.c_dtype, tidx, None, atom, None, gC_mnl, epi_tile,
                    sC))
            # (T2R, T2R_M, T2R_N)
    ...
    def _compute_stages_and_tmem_cols(
        ...
        num_tmem_acc_col_per_stage = utils.get_num_tmem_alloc_cols(
            tCtAcc_stage1, True)
        # Heuristic to decide the number of stages for accumulator
        # Per-CTA budgets: TMEM (512 columns) and SMEM are shared by the CTAs
        # resident on one SM; leave 1 KB of SMEM per CTA for the driver reserve.
        sm100_tmem_columns = cute.arch.get_max_tmem_alloc_cols(
            "sm_100") // ctas_per_sm
        ...
        num_tmem_cols_a_per_stage = cute.round_up(
            (cta_tile_shape_mnk[2] // num_elts_per_tmem_col
             if transform_a_source == tcgen05.OperandSource.TMEM else 0),
            4,
        )
        ...
        smem_capacity = utils.get_smem_capacity_in_bytes(
            "sm_100") // ctas_per_sm - (1024 if ctas_per_sm > 1 else 0)
        ...
        if scale_mode == TransformMode.ConvertOnly:
            ...
            a_scale_k_mode = max(cta_tile_shape_mnk[2] // scale_granularity_k,
                                 1)
            a_scale_m_mode = max(cta_tile_shape_mnk[0] // scale_granularity_m,
                                 1)
        ...
        a_scale_bytes = (a_scale_bytes_per_stage + bytes_per_pipeline_stage
                         ) * scale_load2trans_stage_count
        # The staged tile_group_idx (MAX_TOKEN_TILES int32) lives next to the
        # pipeline buffers and must come out of the stage budget too: without it
        # the tn8 variant allocated 234496 bytes against Thor's 232448 limit.
        tile_expert_bytes = MAX_TOKEN_TILES * 4
        carveout_smem_bytes = (
            bytes_per_pipeline_stage * accumulator_stage_count +
            a_scale_bytes + c_bytes + tile_expert_bytes)
        ...
        transform2mma_stage_count_a_source_tmem_potential = (
            (sm100_tmem_columns - num_tmem_acc_cols) //
            num_tmem_cols_a_per_stage
            if transform_a_source == tcgen05.OperandSource.TMEM else -1)
        if (transform_a_source == tcgen05.OperandSource.TMEM
        ...
        a_load_bytes_per_stage = cute.round_up(
            cute.size_in_bytes(
                a_dtype,
                cute.make_layout(
                    (cta_tile_shape_mnk[0], cta_tile_shape_mnk[2])),
            ),
            smem_buffer_align_bytes,
        )
        ...
        ab_load_bytes_per_stage = (a_load_bytes_per_stage +
                                   b_load_bytes_per_stage +
                                   2 * bytes_per_pipeline_stage)
        a_transform_bytes_per_stage = (cute.round_up(
            cute.size_in_bytes(
            ...
            smem_buffer_align_bytes,
        ) if transform_a_source == tcgen05.OperandSource.SMEM else 0)

        a_transform_bytes_per_stage = (a_transform_bytes_per_stage +
                                       bytes_per_pipeline_stage)
        ...
        transform2mma_stage_count = (
            min(
                transform2mma_stage_count_a_source_tmem_potential,
                transform2mma_stage_count_a_source_smem_potential,
            ) if transform_a_source == tcgen05.OperandSource.TMEM else
            transform2mma_stage_count_a_source_smem_potential)
        load2transform_stage_count = (
            smem_capacity - carveout_smem_bytes -
            (transform2mma_stage_count *
             a_transform_bytes_per_stage)) // ab_load_bytes_per_stage
        if (load2transform_stage_count < 2 or transform2mma_stage_count < 2
        ...
        if use_tma_store:
            # Leftover after every buffer, barrier and the staged tile experts
            # (carveout_smem_bytes), with 1 KB of slack for struct alignment:
            # the upstream formula ignored the barriers and put tn8 1 KB over
            # Thor's 232448-byte limit.
            c_stage_count += (
                smem_capacity - carveout_smem_bytes - 1024 -
                load2transform_stage_count * ab_load_bytes_per_stage -
                transform2mma_stage_count *
                a_transform_bytes_per_stage) // c_bytes_per_stage
...
class Nvfp4A16BlackwellMoeGemmLaunch:
    """Pointer ABI for one (fusion, token-tile, activation-dtype) variant.

    Tensor contract (all extents are runtime Int32; only ``fusion``, the token
    tile and the activation dtype are baked into a variant):

      qweight       [E, N/128, K/64, 128, 32]  packed E2M1 (BLACKWELL_MOE_N128_K64_V1)
      block_scales  [E, N/128, K/64, 128, 4]   raw E4M3
      global_scale  [E] fp32                   verbatim weight_scale_2
      activation    [R_pad, K] with row stride ``activation_ld`` (>= K), FP16/BF16,
                    permuted into expert-contiguous, tile-padded order
      tile_group_idx [R_pad / token_tile] int32 expert per token tile
      num_valid_tiles [1] int32                 device count of non-empty tiles
      relu2_store:  output [R_pad, N] FP16/BF16 (row stride N)
      scatter_add:  output [num_tokens, N] FP16/BF16, pre-zeroed;
                    permuted_idx [R_pad] int32 (token*topK+slot, <0 = pad);
    ...
    def __init__(self,
                 ...
                 token_tile: int,
                 fusion: str,
                 ctas_per_sm: Optional[int] = None):
        self.io_dtype = io_dtype
        self.token_tile = token_tile
        self.fusion = fusion
        # One persistent CTA per SM. ctas_per_sm=2 (half SMEM/TMEM budgets) was
        # tried on Thor: ncu still reported an occupancy limit of one block
        # (384 threads x 168 registers fill the register file; the 1 KB SMEM
        # reserve), so the doubled grid only queued CTAs and ran 2-13% slower.
        # A real two-CTA variant needs smaller setmaxnreg budgets as well; the
        # knob stays for that experiment.
        if ctas_per_sm is None:
            ctas_per_sm = 1
        self.ctas_per_sm = ctas_per_sm
        self.kernel = Nvfp4A16BlackwellMoeGemm(
    ...
    def __call__(
        ...
        tile_group_idx_ptr: cute.Pointer,
        num_valid_tiles_ptr: cute.Pointer,
        permuted_idx_ptr: cute.Pointer,
        topk_weights_ptr: cute.Pointer,
        num_rows_padded: cutlass.Int32,
        activation_ld: cutlass.Int32,
        ...
        # stride calculation before multiplying runtime dimensions.
        rows_64 = cutlass.Int64(num_rows_padded)
        act_ld_64 = cutlass.Int64(activation_ld)
        out_features_64 = cutlass.Int64(out_features)
        in_features_64 = cutlass.Int64(in_features)
        experts_64 = cutlass.Int64(num_experts)
        num_tokens_64 = cutlass.Int64(num_tokens)
        top_k_64 = cutlass.Int64(top_k)
        n_tiles = out_features_64 // _N_TILE
        k_tiles = in_features_64 // _K_TILE
        num_token_tiles = rows_64 // self.token_tile
        expert_plane = out_features_64 * in_features_64  # FP4 codes per expert

        qweight = cute.make_tensor(
            qweight_ptr,
            cute.make_layout(
                ((_N_TILE, n_tiles), (_K_TILE, k_tiles), experts_64),
                stride=(
        ...
        block_scales = cute.make_tensor(
            ...
            cute.make_layout(
                ...
                stride=(
                    (_K_TILE // _SCALE_GROUP,
                     k_tiles * _N_TILE * (_K_TILE // _SCALE_GROUP)),
                    ((0, 1), _N_TILE * (_K_TILE // _SCALE_GROUP)),
                    expert_plane // _SCALE_GROUP,
                ),
            ),
        )
        activation = cute.make_tensor(
            activation_ptr,
            cute.make_layout(
                (rows_64, in_features_64, 1),
                stride=(act_ld_64, 1, rows_64 * act_ld_64),
            ),
        )
        global_scale = cute.make_tensor(
            global_scale_ptr, cute.make_layout((experts_64, ), stride=(1, )))
        tile_group_idx = cute.make_tensor(
            tile_group_idx_ptr,
            cute.make_layout((num_token_tiles, ), stride=(1, )))
        num_valid_tiles = cute.make_tensor(
            num_valid_tiles_ptr, cute.make_layout((1, ), stride=(1, )))
        permuted_idx = cute.make_tensor(
            permuted_idx_ptr, cute.make_layout((rows_64, ), stride=(1, )))
        topk_weights = cute.make_tensor(
            topk_weights_ptr,
            cute.make_layout((num_tokens_64, top_k_64), stride=(top_k_64, 1)),
        )
        if cutlass.const_expr(self.fusion == FUSION_RELU2_STORE):
            # C = permuted intermediate [R_pad, N], M (=N features) contiguous.
            c = cute.make_tensor(
                output_ptr,
                cute.make_layout(
                    (out_features_64, rows_64, 1),
                    stride=(1, out_features_64, rows_64 * out_features_64),
                ),
            )
            out = c
        else:
            # C is only a shape carrier for the fp32 SMEM staging; the real
            # destination is the token output below.
            c = cute.make_tensor(
                cute.recast_ptr(output_ptr, dtype=cutlass.Float32),
                cute.make_layout(
                    (out_features_64, rows_64, 1),
                    stride=(1, out_features_64, rows_64 * out_features_64),
                ),
            ...
            out = cute.make_tensor(
                output_ptr,
                cute.make_layout((num_tokens_64, out_features_64),
                                 stride=(out_features_64, 1)),
            )
        self.kernel(
            qweight,
            block_scales,
...
def make_launch(io_dtype: type[cutlass.Numeric], token_tile: int, fusion: str):
    if token_tile not in _TOKEN_TILES:
        raise ValueError(f"token tile must be one of {_TOKEN_TILES}")
    if fusion not in _FUSIONS:
        raise ValueError(f"fusion must be one of {_FUSIONS}")
    return Nvfp4A16BlackwellMoeGemmLaunch(io_dtype, token_tile, fusion)


def make_trace_args(io_dtype: type[cutlass.Numeric], token_tile: int):
    """Pointer/extent placeholders used to trace one variant for AOT export."""
    activation_ptr = make_ptr(io_dtype,
                              0,
                              cute.AddressSpace.gmem,
                              assumed_align=16)
    qweight_ptr = make_ptr(cutlass.Float4E2M1FN,
                           0,
                           cute.AddressSpace.gmem,
                           assumed_align=16)
    block_scales_ptr = make_ptr(cutlass.Float8E4M3FN,
                                0,
                                cute.AddressSpace.gmem,
                                assumed_align=16)
    global_scale_ptr = make_ptr(cutlass.Float32,
                                0,
                                cute.AddressSpace.gmem,
                                assumed_align=4)
    output_ptr = make_ptr(io_dtype,
                          0,
                          cute.AddressSpace.gmem,
                          assumed_align=16)
    tile_group_idx_ptr = make_ptr(cutlass.Int32,
                                  0,
                                  cute.AddressSpace.gmem,
                                  assumed_align=4)
    num_valid_tiles_ptr = make_ptr(cutlass.Int32,
                                   0,
                                   cute.AddressSpace.gmem,
                                   assumed_align=4)
    permuted_idx_ptr = make_ptr(cutlass.Int32,
                                0,
                                cute.AddressSpace.gmem,
                                assumed_align=4)
    topk_weights_ptr = make_ptr(cutlass.Float32,
                                0,
                                cute.AddressSpace.gmem,
                                assumed_align=4)
    return (
        ...
        permuted_idx_ptr,
        topk_weights_ptr,
        cutlass.Int32(token_tile),  # num_rows_padded
        cutlass.Int32(_K_TILE),  # activation_ld
        cutlass.Int32(_N_TILE),  # out_features
        cutlass.Int32(_K_TILE),  # in_features
        cutlass.Int32(1),  # num_experts
...
def export_variant(args) -> None:
    ...
    launch = make_launch(io_dtype, args.token_tile, args.fusion)
    compiled = cute.compile(launch, *make_trace_args(io_dtype,
                                                     args.token_tile))
...
def _parse_args():
    ...
    parser.add_argument("--fusion", choices=_FUSIONS, required=True)
    parser.add_argument("--export_only", action="store_true")
    parser.add_argument("--output_dir",
                        default="./nvfp4_a16_blackwell_moe_artifacts")
    parser.add_argument("--file_name", default="nvfp4_a16_blackwell_moe")
    parser.add_argument("--function_prefix", default="nvfp4_a16_blackwell_moe")
...
if __name__ == "__main__":
    ...
    if not _args.export_only:
        raise ValueError(
            "This SM110 AOT kernel currently supports --export_only")
