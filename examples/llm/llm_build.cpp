...
struct LLMBuildArgs
{
    ...
    int64_t batchSizeMin{1};
    int64_t batchSizeMax{4};
...
void printUsage(char const* programName)
{
    std::cerr << "Usage: " << programName
              << " [-h] <--onnxPath str> <--enginePath str> [-b or "
                 ...
                 "[--imageTokens] [--minImageTokens] [--maxImageTokens] [--modelType]"
              << std::endl;
    ...
    std::cerr << "  --modelType     Provide the model type, choose from llama,qwen,qwen_vl,qwen2_vl.Required."
              << std::endl;
    std::cerr << "  --imageTokens     Provide the image tokens generated by ViT models for VLM. It's required when "
                 "specified static shape. Default = 888"
              << std::endl;
    std::cerr << "  --minImageTokens     Provide the minimum image tokens generated by ViT models for VLM. It's "
                 "required when specified dynamic shape. Default = 128"
              << std::endl;
    std::cerr << "  --maxImageTokens     Provide the maximum image tokens generated by ViT models for VLM. It's "
...
bool parseLLMBuildArgs(LLMBuildArgs& args, int argc, char* argv[])
{
    static struct option long_options[] = {{"help", no_argument, 0, 'h'}, {"onnxPath", required_argument, 0, 'i'},
        ...
        {"debug", no_argument, 0, 'd'}, {"dynamicShape", no_argument, 0, 'y'}, {"imageTokens", required_argument, 0, 0},
        {"minImageTokens", required_argument, 0, 0}, {"maxImageTokens", required_argument, 0, 0},
        {"modelType", required_argument, 0, 0}, {"abcs", required_argument, 0, 0}, {0, 0, 0, 0}};
    ...
    while ((opt = getopt_long(argc, argv, "hi:o:b:c:s:dy", long_options, &option_index)) != -1)
    {
        ...
        case 0:
            if (strcmp(long_options[option_index].name, "minImageTokens") == 0)
            {
                if (optarg)
                {
            ...
            else if (strcmp(long_options[option_index].name, "maxImageTokens") == 0)
            {
                if (optarg)
                {
            ...
            else if (strcmp(long_options[option_index].name, "imageTokens") == 0)
            {
                if (optarg)
                {
            ...
            else if (strcmp(long_options[option_index].name, "modelType") == 0)
            {
                if (optarg)
                {
                ...
                {
                    std::cerr << "ERROR: model type requires option argument,support llama,qwen,qwen2_vl currently"
                              << std::endl;
                    return false;
                }
            }
...
std::string extractFolderName(std::string const& path)
{
