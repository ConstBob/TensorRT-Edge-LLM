...
void printUsage(char const* programName)
{
    std::cerr
        ...
        << " [-h] <--llmOnnxPath str> <--visualOnnxPath str> <--llmEnginePath str> <--visualEnginePath str> [-b or "
           ...
           "[--imageTokens int] [--minImageTokens int] [--maxImageTokens int] [--modelType str] [--maxLoraRank int]"
        << std::endl;
    ...
    std::cerr << "  --modelType        Provide the VLM model type, choose from [qwen2_vl, qwen2_5_vl, internvl3]. Required."
              << std::endl;
    std::cerr
...
bool parseVLMBuildArgs(VLMBuildArgs& args, int argc, char* argv[])
{
    static struct option long_options[] = {{"help", no_argument, 0, 'h'}, {"debug", no_argument, 0, 'd'},
        {"visualOnly", no_argument, 0, 'V'}, {"llmOnly", no_argument, 0, 'L'},
        {"llmOnnxPath", required_argument, 0, 'i'}, {"llmEnginePath", required_argument, 0, 'o'},
        {"visualOnnxPath", required_argument, 0, 'I'}, {"visualEnginePath", required_argument, 0, 'O'},
        {"batchSize", required_argument, 0, 'b'}, {"maxBatchSize", required_argument, 0, 'B'},
        {"maxInputLen", required_argument, 0, 'c'}, {"maxSeqLen", required_argument, 0, 's'},
        {"dynamicShape", no_argument, 0, 'y'}, {"imageTokens", required_argument, 0, 'M'},
        {"minImageTokens", required_argument, 0, 't'}, {"maxImageTokens", required_argument, 0, 'T'},
        {"modelType", required_argument, 0, 'm'}, {"maxLoraRank", required_argument, 0, 'r'}, {0, 0, 0, 0}};
    ...
    int64_t const& hiddenSizePerHead, int64_t const& imageHiddenSize, int64_t const& mropeDim)
{
    ...
    if (args.modelType == "internvl3")
    {
        // InternVL3 has no mrope_rotary_cos_sin input.
        trtExecCommand = fmtstr(
            ...
            minBatchSize, numKVHeads, hiddenSizePerHead, minImageTokens, imageHiddenSize, optBatchSize, optInputLen,
            optBatchSize, optBatchSize, optBatchSize, numKVHeads, hiddenSizePerHead, optImageTokens, imageHiddenSize, 
            maxBatchSize, maxInputLen, maxBatchSize, maxBatchSize, maxBatchSize, numKVHeads, hiddenSizePerHead, maxImageTokens,
            imageHiddenSize, minBatchSize, minBatchSize, minBatchSize, minBatchSize, numKVHeads, maxLength, hiddenSizePerHead,
            minImageTokens, imageHiddenSize, optBatchSize, optBatchSize, optBatchSize, optBatchSize, numKVHeads, maxLength,
            hiddenSizePerHead, optImageTokens, imageHiddenSize, maxBatchSize, maxBatchSize, maxBatchSize,
            maxBatchSize, numKVHeads, maxLength, hiddenSizePerHead, maxImageTokens, imageHiddenSize);
    }
    else
    {
        trtExecCommand = fmtstr(
...
int buildLLM(VLMBuildArgs const& args)
{
    ...
    {
        LOG_ERROR("Please add image_embeds as model input. Qwen-VL and InternVL3 need (len, hidden_size) size image embeddings.");
    }
    if (mropeDim == 0 && (args.modelType == "qwen2_vl" || args.modelType == "qwen2_5_vl"))
    {
        LOG_ERROR("Please add mrope_rotary_sin_cos as input for Qwen2-VL.");
    }
    ...
    if (args.modelType == "qwen2_vl" || args.modelType == "qwen2_5_vl")
    {
    ...
    int64_t const& minHW, int64_t const& optHW, int64_t const& maxHW)
{
    ...
    else if (args.modelType == "internvl3")
    {
        trtExecCommand += fmtstr(
...
int buildViT(VLMBuildArgs const& args)
{
    ...
    if (args.dynamicShape)
    {
        ...
        // In InternVL3 case as well, downsampling ratio is 0.5 so factor is (1/0.5) ** 2 = 4
    ...
    if ((args.modelType == "qwen2_5_vl" || args.modelType == "qwen2_vl") && (ropeEmbdSize == 0))
    {
...
int main(int argc, char** argv)
{
    ...
    if (args.modelType != "qwen2_vl" && args.modelType != "qwen2_5_vl" && args.modelType != "internvl3")
    {
        LOG_ERROR("Currently only QWen2-VL, Qwen2.5-VL, and InternVL3 is supported for VLM.");
        return EXIT_FAILURE;
    }
