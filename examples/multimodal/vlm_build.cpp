...
void printUsage(char const* programName)
{
    ...
    std::cerr << "  --modelType        Provide the VLM model type, choose from [qwen2_vl, qwen2_5_vl]. Required." << std::endl;
    std::cerr
    ...
    int64_t const& minHW, int64_t const& optHW, int64_t const& maxHW)
{
    ...
    if (args.modelType == "qwne2_vl")
    {
        trtExecCommand += fmtstr(
            ...
            minHW, patchSize, minHW, ropeEmbdSize, minHW, minHW,
            optHW, patchSize, optHW, ropeEmbdSize, optHW, optHW,
            maxHW, patchSize, maxHW, ropeEmbdSize, maxHW, maxHW);
    }
    else
    {
        trtExecCommand += fmtstr(
            "--minShapes=input:%ldx%ld,rotary_pos_emb:%ldx%ld,attention_mask:1x%ldx%ld,window_attention_mask:1x%ldx%ld,window_index:%ld,reverse_window_index:%ld "
            "--optShapes=input:%ldx%ld,rotary_pos_emb:%ldx%ld,attention_mask:1x%ldx%ld,window_attention_mask:1x%ldx%ld,window_index:%ld,reverse_window_index:%ld "
            "--maxShapes=input:%ldx%ld,rotary_pos_emb:%ldx%ld,attention_mask:1x%ldx%ld,window_attention_mask:1x%ldx%ld,window_index:%ld,reverse_window_index:%ld ",
            minHW, patchSize, minHW, ropeEmbdSize, minHW, minHW, minHW, minHW, minHW / 4, minHW / 4,
            optHW, patchSize, optHW, ropeEmbdSize, optHW, optHW, optHW, optHW, optHW / 4, optHW / 4,
            maxHW, patchSize, maxHW, ropeEmbdSize, maxHW, maxHW, maxHW, maxHW, maxHW / 4, maxHW / 4);
    }
...
int main(int argc, char** argv)
{
    ...
    {
        LOG_ERROR("Currently only QWen2-VL and Qwen2.5-VL is supported for VLM.");
        return EXIT_FAILURE;
    }
