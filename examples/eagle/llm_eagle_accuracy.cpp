...
bool parseLLMEagleAccuracyArgs(LLMEagleAccuracyArgs& args, int argc, char* argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "he:E:t:D:d:i", long_options, nullptr)) != -1)
    {
        switch (opt)
        {
    ...
    int32_t topK, int32_t maxPathLen)
{
    ...
    for (auto const& subject : subjects)
    {
        ...
        for (auto const& data : testData)
        {
            ...
            auto const& hostLogits = eagle->getLastHostLogits();
            int bestIdx = 0;
...
{
    LLMEagleAccuracyArgs args;
    if ((argc < 2) || (!parseLLMEagleAccuracyArgs(args, argc, argv)))
    {
        printUsage(argv[0]);
        return EXIT_FAILURE;
    }
    ...
    mmluAccuracy(args.enginePath, args.eagleEnginePath, args.datasetPath, tokenizer.get(), generationConfig, args.debug,
        args.isEagle3, args.maxDecodingTokens, args.topK, args.maxPathLen);
    return EXIT_SUCCESS;
};