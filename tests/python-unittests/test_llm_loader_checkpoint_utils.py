...
"""Tests for ``experimental/llm_loader/checkpoint/checkpoint_utils.py`` —
specifically the rope recovery path.

Transformers v5 moved RoPE config from ``rope_scaling`` to ``rope_parameters``
(with the old key left as ``null`` for backward-compat), and for VLMs the
settings live inside ``text_config``. The C++ runtime's ``collectRopeConfig``
...
# Load llm_loader from the experimental tree without installing it.
_REPO_ROOT = os.path.normpath(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", ".."))
_EXPERIMENTAL = os.path.join(_REPO_ROOT, "experimental")
if _EXPERIMENTAL not in sys.path:
    sys.path.insert(0, _EXPERIMENTAL)

try:
    from llm_loader.checkpoint.checkpoint_utils import \
        load_checkpoint_config_dicts
except ImportError as exc:  # pragma: no cover
    pytest.skip(f"llm_loader not importable: {exc}", allow_module_level=True)
...
def test_legacy_rope_scaling_preserved_unchanged():
    """Pre-v5 HF configs (and non-MRope v5 configs) still work: if
    rope_scaling is already populated, the patch is a no-op."""
    with tempfile.TemporaryDirectory() as td:
        _write_config(
            td, {
                "model_type": "qwen2",
