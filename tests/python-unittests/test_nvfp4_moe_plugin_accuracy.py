...
Marlin-unpacked reference; HF dense refs (NumPy / torch) are cross-checks only for the W4A16 (FP16 hidden) path.
W4A4 cases feed INT8 NVFP4-packed activations to TensorRT; the reference uses the same pack → CPU-dequant path to
FP16 hidden states, then :meth:`NemotronHMoEReference.reference_from_packed_plugin_state`. Router logits are explicit
engine inputs (see :func:`serialize_nvfp4_moe_engine_with_explicit_router`), matching the C++ harness style.
...
(flush to stderr; use ``pytest -s`` / ``--capture=no`` to see it live). W4A4 cases reshape dense refs to
``[batch, seq, hidden]`` for the same layout as TRT and print a short W4A4 note plus TRT vs NumPy Marlin stats.
"""
        ...
        *,
        ...
        if cfg.print_header:
            ...
            if w4a4_dequant_hidden:
                print(
                    "  note: dense numpy_hf / torch refs use CPU-dequantized NVFP4 activations (W4A4 packed path); "
                    "TRT assert uses NumPy Marlin unpack ref.",
                    file=out,
                    flush=True,
                )
...
def build_nvfp4_moe_engine_trt_api(
    ...
    *,
    ...
    When ``w4a4_activation`` is True, ``hidden_states`` is INT8 ``(B,S,H/2)`` NVFP4-packed activations,
    and ``hidden_block_scale`` / ``hidden_global_scale`` are runtime inputs (Marlin tile scales).
    Router logits **must** be explicit (``explicit_router_logits=True``); INT8 activations cannot drive
    the gate matmul.
    ...
    if w4a4_activation and not explicit_router_logits:
        raise ValueError(
            "w4a4_activation=True requires explicit_router_logits=True")
    ...
    if verbose:
        ...
        hid_mode = f"INT8 NVFP4 ({b},{s},{h // 2})" if w4a4_activation else f"FP16 ({b},{s},{h})"
        print(
            f"[TRT API{tag}] Building engine (STRONGLY_TYPED, add_plugin_v3)  "
            f"hidden_states={hid_mode}  router={r_mode}")
    ...
    pfc = trt.PluginFieldCollection([
        trt.PluginField("num_experts", np.array([e_ct], dtype=np.int32),
                        trt.PluginFieldType.INT32),
        trt.PluginField("top_k", np.array([top_k], dtype=np.int32),
                        trt.PluginFieldType.INT32),
        trt.PluginField("hidden_size", np.array([h], dtype=np.int32),
                        trt.PluginFieldType.INT32),
        trt.PluginField("moe_inter_size", np.array([inter], dtype=np.int32),
                        trt.PluginFieldType.INT32),
        trt.PluginField("activation_type", np.array([f_act], dtype=np.int32),
                        trt.PluginFieldType.INT32),
        trt.PluginField("quantization_group_size",
    ...
    if explicit_router_logits:
        ...
        if w4a4_activation:
            raise RuntimeError(
                "internal: w4a4_activation requires explicit_router_logits")
    ...
    if explicit_router_logits:
        rt_shape = (b * s, e_ct)
        profile.set_shape("router_logits", rt_shape, rt_shape, rt_shape)
...
def serialize_nvfp4_moe_engine_with_explicit_router(
    ...
    *,
    ...
    return build_nvfp4_moe_engine_trt_api(
        ...
        w4a4_activation=w4a4_activation,
    )
...
def run_nvfp4_w4a16_moe_plugin_accuracy_case(
    *,
    ...
    ref_torch_fp16 = NemotronHMoEReference.routed_experts_from_router_logits_via_hf_experts(
        moe,
        hidden_bsh,
        logits_t,
        tk,
    ...
    ref_torch_marlin_np = NemotronHMoEReference.routed_experts_from_router_logits_via_marlin_unpacked_weights(
        hidden_bsh,
        logits_t,
        tk,
        fc_up_q=mod_cpu.fc_up_qweights.numpy(),
        fc_up_bs=mod_cpu.fc_up_blocks_scale.numpy(),
        fc_dn_q=mod_cpu.fc_down_qweights.numpy(),
        fc_dn_bs=mod_cpu.fc_down_blocks_scale.numpy(),
        fc_up_gs=mod_cpu.fc_up_global_scale.numpy(),
        fc_dn_gs=mod_cpu.fc_down_global_scale.numpy(),
        hidden_size=h,
        moe_inter_size=inter,
        num_experts=e_ct,
        activation_type=int(mod_cpu.activation_type),
        case=f"{case}_torch_marlin_ref",
    )
    NemotronHMoEReference.assert_marlin_unpacked_numpy_matches_torch(
        ref_marlin_np, ref_torch_marlin_np,
        f"{case}_numpy_vs_torch_marlin_unpack")

    if PRINT_TRT_VS_TORCH_DIST:
        NemotronHMoEReference.print_cross_check_output_distribution(
            trt_out,
            ref_torch_fp32,
            ref_np.astype(np.float32),
            case,
            numpy_marlin_unpack_ref_fp32=ref_marlin_np.astype(np.float32),
            torch_marlin_unpack_ref_fp32=ref_torch_marlin_np.astype(
                np.float32),
            torch_hf_fp16_ref_fp32_bsh=ref_torch_fp16.astype(np.float32),
            print_config=PRINT_TRT_VS_TORCH_DIST_PRINT_CONFIG,
        )

    NemotronHMoEReference.assert_numpy_dense_matches_torch_reference(
        ref_np.astype(np.float32), ref_torch_fp32,
        f"{case}_numpy_hf_vs_torch_fp32")
    NemotronHMoEReference.assert_fp16_ref_tracks_fp32_ref(
        ref_torch_fp32, ref_torch_fp16, f"{case}_torch_fp16_vs_fp32")
...
def run_nvfp4_w4a4_moe_plugin_accuracy_case(
    *,
    ...
    """One accuracy scenario (W4A4 packed activations): same cross-checks as W4A16; TRT uses INT8 + scales.

    References use CPU-dequant FP16 hidden (matching the packed path) for HF dense + Marlin unpack.
    """
    check_requirements()
    assert trt is not None
    batch = _NVFP4_MOE_FAST_BATCH
    ...
    trt_out = execute_trt_engine(
        eng,
        {
            "router_logits": np.ascontiguousarray(logits_np.astype(
                np.float32)),
            "hidden_states": np.ascontiguousarray(act_pl),
            "hidden_block_scale": np.ascontiguousarray(act_bs),
            "hidden_global_scale": np.ascontiguousarray(act_gs),
        },
        stream,
        device=device,
        verbose=False,
    ...
    if PRINT_TRT_VS_TORCH_DIST:
        NemotronHMoEReference.print_cross_check_output_distribution(
            trt_out,
            ref_torch_fp32,
            np.asarray(ref_np, dtype=np.float32).reshape(batch, seq, h),
            case,
            numpy_marlin_unpack_ref_fp32=np.asarray(ref_marlin_np,
    ...
    NemotronHMoEReference.assert_numpy_dense_matches_torch_reference(
        ref_np.astype(np.float32), ref_torch_fp32,
        f"{case}_numpy_hf_vs_torch_fp32")
    # Skip :meth:`assert_fp16_ref_tracks_fp32_ref` for W4A4: dense FP32 matmul already matches NumPy above;
    # FP16 vs FP32 routing masses can disagree on isolated dims under extreme dequant dynamic range (top-k>1)
    # while large-|elem| diffs elsewhere dominate ``argmax(diff)`` in failure diagnostics.
    NemotronHMoEReference.assert_trt_output_matches_nvfp4_marlin_numpy_reference(
        trt_out, ref_marlin_np.astype(np.float32), case)
...
def test_nvfp4_w4a4_moe_plugin_accuracy(
    ...
    """Parametrized plugin accuracy (B=2 fixed; S=len(expert_rows)/B; H=384,I=768,E=8); W4A4 packed activations."""
    check_requirements()
    dev = torch.device("cuda", torch.cuda.current_device())
    expert = np.asarray(expert_rows, dtype=np.int32)
    score = np.asarray(score_rows, dtype=np.float32)
