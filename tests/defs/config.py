...
class TestConfig:
    ...
    def get_quantized_draft_checkpoint_dir_name(self) -> Optional[str]:
        ...
        if base_model_name not in draft_models_map:
            return None
        if self.draft_model_id not in draft_models_map[base_model_name]:
            return None
        draft_dir_name = draft_models_map[base_model_name][self.draft_model_id]
        return f"{draft_dir_name}-{quant_suffix}"
    ...
    def _resolve_torch_model_dir(self, *, prefer_hub_quant: bool) -> str:
        ...
        if use_torch_base:
            ...
            if base_model_name in LLM_MODELS_DIR_MAP:
                search_dir = self.llm_models_dir
                base_entry = LLM_MODELS_DIR_MAP[base_model_name]
                base_candidates = ([base_entry] if isinstance(base_entry, str)
                                   else list(base_entry))
                candidates = [self.model_name] + base_candidates
            elif base_model_name in GPTQ_MODELS_DIR_MAP:
                search_dir = self.edgellm_data_dir
                base_entry = GPTQ_MODELS_DIR_MAP[base_model_name]
                base_candidates = ([base_entry] if isinstance(base_entry, str)
                                   else list(base_entry))
                candidates = [self.model_name] + base_candidates
            else:
        ...
        if prefer_hub_quant:
            ...
            if checkpoint_dir_name and checkpoint_dir_name not in candidates:
    ...
    def get_onnx_base_dir(self) -> str:
        ...
        return os.path.join(self.onnx_dir,
    ...
    def get_draft_onnx_dir(self) -> str:
        ...
        if self.is_mtp:
            ...
            return os.path.join(self.get_onnx_base_dir(),
                                f"mtp-draft-{self.get_quantized_model_id()}")
        return os.path.join(self.get_onnx_base_dir(),
