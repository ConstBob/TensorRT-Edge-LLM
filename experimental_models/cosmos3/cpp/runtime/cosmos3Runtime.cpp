...
std::vector<float> Cosmos3Runtime::generatePolicy(
    rt::Tensor const& pixelValues, rt::Tensor const& inputsEmbeds, rt::Tensor const* uncondEmbeds,
    std::vector<float> const& currentState, cudaStream_t stream)
{
    ...
    if (mGuidance != 1.0F && uncondEmbeds != nullptr)
    {
        ...
        return mPolicyRunner->generate(
            condLatent, mUndK, mUndV, mUndKUncond, mUndVUncond, currentState, stream);
    }
    if (mGuidance != 1.0F)
    {
