...
__all__ = [
    ...
    "repack_nvfp4_qwen3_moe_experts_preserve_quant",
...
def _as_int8_checkpoint_weight(raw_weight: torch.Tensor) -> torch.Tensor:
    """Return checkpoint FP4 bytes as contiguous int8 without changing bits."""
    weight = raw_weight.detach().cpu()
    if weight.dtype == torch.uint8:
        weight = weight.view(torch.int8)
    if weight.dtype != torch.int8:
        raise TypeError(
            f"NVFP4 weights must be int8/uint8, got {weight.dtype}")
    return weight.contiguous()


def _scalar_from_tensor(raw: torch.Tensor, name: str) -> float:
    values = raw.detach().reshape(-1).float()
    if values.numel() != 1:
        raise ValueError(
            f"{name} must be scalar-like, got shape {tuple(raw.shape)}")
    return float(values[0].item())


def _matching_scalar(left: torch.Tensor, right: torch.Tensor,
                     name: str) -> float:
    """Return a shared scalar when gate/up ModelOpt metadata match."""
    left_value = _scalar_from_tensor(left, f"{name}.left")
    right_value = _scalar_from_tensor(right, f"{name}.right")
    tolerance = max(1.0e-12, 1.0e-6 * max(abs(left_value), abs(right_value)))
    if abs(left_value - right_value) > tolerance:
        raise ValueError(
            f"{name} must match for fused SwiGLU FC1: {left_value} vs {right_value}"
        )
    return left_value


def _interleave_qwen3_swiglu_rows(
    gate_rows: torch.Tensor,
    up_rows: torch.Tensor,
    hidden_size: int,
    moe_inter_size: int,
) -> torch.Tensor:
    """Interleave row-major gate/up tensors in the SM100/101/110 FC1 layout."""
    if tuple(gate_rows.shape) != tuple(up_rows.shape):
        raise ValueError(
            f"gate rows shape {tuple(gate_rows.shape)} != up rows shape {tuple(up_rows.shape)}"
        )
    if gate_rows.shape[0] != moe_inter_size:
        raise ValueError(
            f"gate/up rows first dim {gate_rows.shape[0]} != moe_inter_size {moe_inter_size}"
        )
    swiglu_interleave_rows = 64
    if moe_inter_size % swiglu_interleave_rows != 0:
        raise ValueError(
    ...
    n_chunks = moe_inter_size // swiglu_interleave_rows
    row_tail = gate_rows.shape[1:]
    up_chunks = up_rows.reshape(n_chunks, swiglu_interleave_rows, *row_tail)
    gate_chunks = gate_rows.reshape(n_chunks, swiglu_interleave_rows,
                                    *row_tail)
    return torch.stack([up_chunks, gate_chunks],
                       dim=1).reshape(2 * moe_inter_size, *row_tail)


def _concat_qwen3_swiglu_rows(
    gate_rows: torch.Tensor,
    up_rows: torch.Tensor,
    hidden_size: int,
    moe_inter_size: int,
) -> torch.Tensor:
    """Concatenate row-major gate/up tensors in the SM12x FC1 layout."""
    if tuple(gate_rows.shape) != tuple(up_rows.shape):
        raise ValueError(
            f"gate rows shape {tuple(gate_rows.shape)} != up rows shape {tuple(up_rows.shape)}"
        )
    ...
    if hidden_size <= 0:
        raise ValueError(f"hidden_size ({hidden_size}) must be positive")
    return torch.cat([up_rows, gate_rows], dim=0).contiguous()


def repack_nvfp4_qwen3_moe_experts_preserve_quant(
    ...
    """Pack ModelOpt NVFP4 experts while preserving checkpoint quantization.

    This is the strict ModelOpt path for Gemma4/DiffusionGemma NVFP4
    checkpoints. Unlike :func:`repack_nvfp4_qwen3_moe_experts`, it does not
    dequantize to dense and re-quantize. It only changes the FC1 row layout
    (up/gate interleave or concat), swizzles FP8 block scales to CuTeDSL's
    physical layout, and forwards the checkpoint ``weight_scale_2`` and
    ``input_scale`` tensors as plugin alphas / activation scale-of-scale values.
    """
    from ..models.linear import \
        is_nvfp4_linear  # local import to avoid circular dep
    ...
    if fc1_layout == "interleave":
        build_fc1_rows = _interleave_qwen3_swiglu_rows
    elif fc1_layout == "concat":
        build_fc1_rows = _concat_qwen3_swiglu_rows
    else:
        raise ValueError(
    ...
    for expert in experts:
        ...
        expected_gate_weight = (moe_inter_size, hidden_size // 2)
        expected_gate_scale = (moe_inter_size, hidden_size // group_size)
        expected_down_weight = (hidden_size, moe_inter_size // 2)
        expected_down_scale = (hidden_size, moe_inter_size // group_size)
        if tuple(gate.weight.shape) != expected_gate_weight:
            raise ValueError(
                f"gate weight shape {tuple(gate.weight.shape)} != {expected_gate_weight}"
            )
        if tuple(up.weight.shape) != expected_gate_weight:
            raise ValueError(
                f"up weight shape {tuple(up.weight.shape)} != {expected_gate_weight}"
            )
        if tuple(gate.weight_scale.shape) != expected_gate_scale:
            raise ValueError(
                f"gate scale shape {tuple(gate.weight_scale.shape)} != {expected_gate_scale}"
            )
        if tuple(up.weight_scale.shape) != expected_gate_scale:
            raise ValueError(
                f"up scale shape {tuple(up.weight_scale.shape)} != {expected_gate_scale}"
            )
        if tuple(down.weight.shape) != expected_down_weight:
            raise ValueError(
                f"down weight shape {tuple(down.weight.shape)} != {expected_down_weight}"
            )
        if tuple(down.weight_scale.shape) != expected_down_scale:
            raise ValueError(
                f"down scale shape {tuple(down.weight_scale.shape)} != {expected_down_scale}"
            )

        gate_weight = _as_int8_checkpoint_weight(gate.weight)
        up_weight = _as_int8_checkpoint_weight(up.weight)
        down_weight = _as_int8_checkpoint_weight(down.weight)
        fc1_weight = build_fc1_rows(gate_weight, up_weight, hidden_size,
                                    moe_inter_size)

        gate_sf = torch.from_numpy(_sf_bytes_from_checkpoint(
            gate.weight_scale))
        up_sf = torch.from_numpy(_sf_bytes_from_checkpoint(up.weight_scale))
        fc1_sf_linear = build_fc1_rows(gate_sf, up_sf, hidden_size,
                                       moe_inter_size).numpy()
        down_sf_linear = _sf_bytes_from_checkpoint(down.weight_scale)

        fc1_sf = _swizzle_nvfp4_mma_scales(fc1_sf_linear, 2 * moe_inter_size,
                                           hidden_size // group_size)
        fc2_sf = _swizzle_nvfp4_mma_scales(down_sf_linear, hidden_size,
                                           moe_inter_size // group_size)

        fc1_qweights.append(fc1_weight.contiguous())
        fc1_blocks_scale.append(torch.from_numpy(fc1_sf))
        fc1_alpha.append(
            _matching_scalar(gate.weight_scale_2, up.weight_scale_2,
                             "gate/up weight_scale_2"))
        fc2_qweights.append(down_weight.contiguous())
        fc2_blocks_scale.append(torch.from_numpy(fc2_sf))
        fc2_alpha.append(
            _scalar_from_tensor(down.weight_scale_2, "down.weight_scale_2"))
        input_global_scale.append(
            _matching_scalar(gate.input_scale, up.input_scale,
                             "gate/up input_scale"))
        down_input_scale.append(
            _scalar_from_tensor(down.input_scale, "down.input_scale"))
    ...
    return (torch.stack(fc1_qweights,
            ...
            torch.tensor(input_global_scale, dtype=torch.float32),
            torch.tensor(down_input_scale, dtype=torch.float32))
