...
def main():
    ...
    model = AutoModelForCausalLM.from_pretrained(args.model_dir).to(float_dtype_to_torch(args.dtype))
    shape_dict = {}
    ...
    for name, param in model.named_parameters():
        ...
        print(f"{name}: shape: {shape_dict[name]}; type: {param.dtype}; saved to {name}.bin")
