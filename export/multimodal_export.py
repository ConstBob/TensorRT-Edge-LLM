...
def export_qwen2_vl_visual(hf_model, output_dir):
    ...
    rotary_pos_emb = torch.randn((hw, rotary_pos_emb_dim), dtype=torch.float32, device=model.device)
    attention_mask = torch.randn((1, hw, hw), dtype=torch.float16, device=model.device)
...
def export_qwen2_5_vl_visual(hf_model, output_dir):
    from transformers.models.qwen2_5_vl.modeling_qwen2_5_vl import (
        ...
        Qwen2_5_VLVisionAttention, apply_rotary_pos_emb_vision)
    ...
    rotary_pos_emb = torch.randn((hw, rotary_pos_emb_dim), dtype=torch.float32, device=model.device)
    attention_mask = torch.randn((1, hw, hw), dtype=torch.float16, device=model.device)
    window_attention_mask = torch.randn((1, hw, hw), dtype=torch.float16, device=model.device)
    
    window_index = torch.arange(hw // 4, dtype=torch.int64, device=model.device)
    ...
    torch_to_onnx(
        ...
        output_dir,
        "model.onnx",
        input_names=["input", "rotary_pos_emb", "attention_mask", "window_attention_mask", "window_index", "reverse_window_index"],
        output_names=["output"],
        dynamic_axes=dynamic_axes,
    )
