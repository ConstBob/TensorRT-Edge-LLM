...
private:
    ...
    void formatPatch(rt::imageUtils::ImageData const& image, std::vector<std::vector<int64_t>>& imageGridTHWs,
        std::vector<int64_t>& imageTokenLengths, std::vector<int64_t>& cuSeqlens, cudaStream_t stream);
    ...
    rt::Tensor mWindowIndex{};                    //!< Window index mapping for window attention for Qwen2.5-VL
    rt::Tensor mReverseWindowIndex{};             //!< Reverse window index mapping for Qwen2.5-VL
    rt::Tensor mFastPosEmbIdx{};                  //!< Fast position embeddings index tensor for Qwen3-VL
    rt::Tensor mFastPosEmbWeight{};               //!< Fast position embeddings weight tensor for Qwen3-VL
    std::vector<rt::Tensor> mDeepstackFeatures{}; //!< Deepstack features tensors for Qwen3-VL
