    ...
    kernels::ShmAllReduceState* state, int32_t rank, FusedAllReduceExecutionContext const& context)
{
    ...
    {
        LOG_ERROR(
            "FusedNvfp4GemmAllReducePlugin: FP16 CuTe DSL GEMM (SHM FP16 path) failed: %s", cudaGetErrorString(error));
        return AllReduceExecutionStatus::kFailure;
    }

    cudaError_t const allReduceError = context.outputElements <= kernels::kShmSingleCtaMaxElements
        ? kernels::shmAllReduceExecFused(
              state, context.outputs[kOUT_TENSOR_IDX], context.outputElements, rank, context.stream)
        : kernels::shmAllReduceMultiCtaFp16(
              state, context.outputs[kOUT_TENSOR_IDX], context.outputElements, rank, context.stream);
    if (allReduceError != cudaSuccess)
    {
        LOG_ERROR(
    ...
    ShmAllReducePathRegistration const& registration, FusedAllReduceExecutionContext const& context)
{
    ...
    // Keep the FP8 transport optimization on attention output projections. Quantizing both
    // attention and MLP row-parallel partials compounds E4M3 error through every decoder layer;
    // MLP output projections therefore retain the FP16 SHM path.
    bool const isAttentionOutputProjection
        = static_cast<int64_t>(context.k) * context.tpSize == static_cast<int64_t>(context.n);
    bool const useSmallPath = isAttentionOutputProjection && state->fp8SmallPathElementThreshold > 0
