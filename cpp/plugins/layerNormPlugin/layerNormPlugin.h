...
//! TensorRT V3 plugin for the CuTe DSL LayerNorm kernels.
//!
//! The plugin accepts homogeneous FP16 or BF16 x, gamma, and beta tensors and produces y with x's shape and type.
//! Mean, variance, normalization, and affine arithmetic use FP32 before the result is cast to the activation type.
//! The implementation uses no workspace.
...
public:
    ...
    LayerNormPlugin(std::string const& name, nvinfer1::PluginFieldCollection const* fields);
    LayerNormPlugin() = delete;
    LayerNormPlugin(LayerNormPlugin const&) = delete;
    ~LayerNormPlugin() noexcept override = default;
...
//! Creator for LayerNormPlugin version 1.
