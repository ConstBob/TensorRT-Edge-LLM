    ...
    , mLocalDevices(config.localDevices)
{
    ...
    // {$edge-llm-internal-release begin}
    if (!config.shmAllReduceConfig.enableShmAllReduce)
    {
        LOG_INFO("SHM AllReduce plugin registration disabled.");
    }
    else
    {
        try
        ...
        {
            LOG_WARNING("SHM AllReduce plugin registration skipped; using NCCL fallback: %s", e.what());
        }
    }
    LOG_INFO("TP plugin resources initialized: tpSize=%d, shm=%s, nccl=%s", config.tpSize,
        hasAllReducePath(AllReducePathType::kShm) ? "registered" : "unavailable",
        hasAllReducePath(AllReducePathType::kNccl) ? "registered" : "unavailable");
    // {$edge-llm-internal-release end}
}
