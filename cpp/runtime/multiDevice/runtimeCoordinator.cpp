...
void pinThreadToNumaNode(int32_t rank, int32_t worldSize)
{
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    int32_t const hardwareThreads = static_cast<int32_t>(std::thread::hardware_concurrency());
    int32_t const coresPerNode = std::max(1, hardwareThreads / 2);
    int32_t const core = std::min(rank * coresPerNode, CPU_SETSIZE - 1);
    CPU_SET(core, &cpuset);

    int32_t const rc = pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpuset);
    if (rc == 0)
    {
        LOG_INFO("[parallel rank %d/%d] Pinned worker thread to core %d", rank, worldSize, core);
    }
    else
    {
        LOG_WARNING("[parallel rank %d/%d] Failed to pin worker thread to core %d (rc=%d)", rank, worldSize, core, rc);
    }
}

int64_t inferTensorShmAllReduceMaxElements(std::string const& engineDir, ParallelConfig const& parallelConfig)
{
    parallel_artifacts::RankArtifactContext const artifactContext{parallelWorldSize(parallelConfig),
        /*globalRank=*/0};
    std::filesystem::path const configPath
        = std::filesystem::path(engineDir) / parallel_artifacts::configFileName(artifactContext);
    std::ifstream configFile(configPath);
    if (!configFile.is_open())
    {
        throw std::runtime_error("Failed to open parallel engine config file: " + configPath.string());
    }

    nlohmann::json const config = nlohmann::json::parse(configFile);
    nlohmann::json const& builderConfig = config.at("builder_config");
    int64_t const maxBatchSize = builderConfig.value("max_batch_size", 0);
    int64_t const maxInputLen = builderConfig.value("max_input_len", 0);
    int64_t const hiddenSize = config.value("hidden_size", builderConfig.value("hidden_size", 0));
    if (maxBatchSize <= 0 || maxInputLen <= 0 || hiddenSize <= 0)
    {
        throw std::runtime_error(
            "Parallel engine config does not contain positive max_batch_size, max_input_len, and hidden_size.");
    }
    if (maxBatchSize > std::numeric_limits<int64_t>::max() / maxInputLen
        || maxBatchSize * maxInputLen > std::numeric_limits<int64_t>::max() / hiddenSize)
    {
        throw std::runtime_error("Parallel engine maximum input shape exceeds the supported SHM capacity range.");
    }

    return maxBatchSize * maxInputLen * hiddenSize;
}
...
{
    // {$edge-llm-internal-release begin}
    bool const hasMultiRankGroup = std::any_of(mRankPlans[0].groups.begin(), mRankPlans[0].groups.end(),
        [](ParallelGroupConfig const& groupConfig) { return groupConfig.size > 1; });
    ShmAllReduceConfig shmAllReduceConfig = mParallelConfig.shmAllReduceConfig;
    if (hasMultiRankGroup && shmAllReduceConfig.enableShmAllReduce && shmAllReduceConfig.shmMaxElements <= 0)
    {
        try
        {
            shmAllReduceConfig.shmMaxElements = inferShmAllReduceMaxElements();
        }
        catch (std::exception const& e)
        {
            LOG_WARNING("Failed to infer SHM AllReduce capacity: %s. SHM registration will be skipped.", e.what());
        }
    }
    // {$edge-llm-internal-release end}
    ...
    for (ParallelGroupConfig const& groupConfig : mRankPlans[0].groups)
    {
        ...
        // {$edge-llm-internal-release begin}
        resourceConfig.shmAllReduceConfig = shmAllReduceConfig;
        // {$edge-llm-internal-release end}
    ...
    for (int32_t const rank : mLocalRanks)
    {
        mWorkers.emplace_back([this, rank]() {
            ...
            try
            {
                CUDA_CHECK(cudaSetDevice(deviceForRank(rank)));
                // {$edge-llm-internal-release begin}
                pinThreadToNumaNode(rank, mWorldSize);
                // {$edge-llm-internal-release end}
            }
            catch (std::exception const& e)
            {
...
// {$edge-llm-internal-release begin}
int64_t RuntimeCoordinator::inferShmAllReduceMaxElements() const
{
    return inferTensorShmAllReduceMaxElements(mConfig.engineDir, mParallelConfig);
}
