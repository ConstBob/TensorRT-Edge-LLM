...
struct MtpLayerInfo
{
    void* recurrentDst; //!< Main recurrent state pool [residentPoolRows, recElements] FP32
    void* recurrentSrc; //!< Intermediate recurrent state buffer [batchSize, verifyTreeSize, recElements] FP32
    void* convDst;      //!< Main conv state pool [residentPoolRows, convElements] FP16
...
/// For each (layer, batch): dst[stateIndices[b], :] = src[b, acceptLengths[b] - 1, :].
/// Skip if acceptLengths[b] <= 0; otherwise commit the selected endpoint, including the final verify row.
...
void mtpScatterRecurrentStates(MtpLayerInfo const* deviceLayerInfos, int32_t numLayers, int32_t activeBatchSize,
    ...
    int32_t const* stateIndices, cudaStream_t stream, bool recurrentStateIsHalf = false);
...
/// dst[stateIndices[b], :] = src[b, nodeId, :].
