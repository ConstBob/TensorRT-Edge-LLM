...
__global__ void topKStage1(T const* __restrict logProbs, T const* const* __restrict logProbsPtrs, T* tmpLogProbs,
    std::int32_t* topKTmpIdBuf, T* topKTmpValBuf, FinishedState const* finished, std::int32_t maxTopK,
    std::int32_t const* topKs, std::int32_t vocabSize, std::int32_t const* endIds, bool const* skipDecode,
    std::int32_t const* batchSlots, std::int32_t const* tokensPerStep, std::int32_t maxTokensPerStep)
{
...
__global__ void topKStage2Sampling(std::int32_t const* __restrict topKTmpIdBuf, T* topKTmpValBuf,
    std::int64_t** idsPtrs, std::int64_t* ids, std::int32_t* sequenceLengths, FinishedState const* finishedInput,
    FinishedState* finishedOutput, float* cumLogProbs, float* outputLogProbs, std::int32_t maxTopK,
    std::int32_t const* topKs, float topP, float const* topPs, curandState_t* curandState, std::int32_t const* endIds,
    std::int32_t vocabSize, bool const* skipDecode, std::int32_t const* batchSlots, std::int32_t maxBatchSize,
    bool normalizeLogProbs, bool logitHasProbs, std::int32_t const* tokensPerStep, std::int32_t maxTokensPerStep,
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            topKStage2Sampling<T, BLOCK_SIZE_2_, BLOCKS_PER_BEAM_>                                                     \
                <<<grid, block, K_MAX * sizeof(std::int32_t) + K_MAX * sizeof(float), stream>>>(topKTmpIdBuf,          \
