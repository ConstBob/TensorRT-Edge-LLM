...
template <typename T>
bool allOfBatchSlots(std::int32_t const* batchSlotsHost, T const* data, std::int32_t batchSize, T value)
{
    return std::all_of(batchSlotsHost, batchSlotsHost + batchSize, [&](std::int32_t b) { return data[b] == value; });
}

template <int32_t TOP_K_MAX>
__global__ void setupTopKRuntimeArgs(std::int32_t batchSize, std::int32_t topK, std::int32_t* topKs,
    std::int32_t topKsSize, float topP, float* topPs, std::int32_t topPsSize, bool* skipDecode,
    std::int32_t const* batchSlots)
{
    auto const index = static_cast<std::int32_t>(blockIdx.x * blockDim.x + threadIdx.x);
    for (auto bi = index; bi < batchSize; bi += static_cast<std::int32_t>(gridDim.x * blockDim.x))
    {
...
void TopKSamplingLayer<T>::allocateBuffer(std::int32_t const batchSize)
{
    ...
    mRuntimeTopKDevice = TensorWrapper(int32Buffer, {batchSize}, TRTDataType<std::int32_t>::value);
    mRuntimeTopPDevice = TensorWrapper(floatBuffer, {batchSize}, TRTDataType<float>::value);
    mSkipDecodeDevice = TensorWrapper(boolBuffer, {batchSize}, TRTDataType<bool>::value);
    mSetupWorkspaceDevice = TensorWrapper(int32Buffer + batchSize, {batchSize}, TRTDataType<std::int32_t>::value);

    mSkipDecodeHost = TensorWrapper(boolHostBuffer, {batchSize}, TRTDataType<bool>::value);
}
...
template <typename T>
void TopKSamplingLayer<T>::setup(std::int32_t batchSize, std::int32_t beamWidth, BufferConstPtr batchSlots,
    std::shared_ptr<BaseSetupParams> const& baseSetupParams)
{
    ...
    auto runtimeTopK = setupParams->runtimeTopK.value_or(std::vector<std::int32_t>(batchSize, defaultTopK));
    ...
    for (auto& topP : runtimeTopP)
    {
        ...
        {
            printf("TopP (%f) is out of range ([0.0, 1.0f]). Clip to closest number.", topP);
    ...
    for (auto& topK : runtimeTopK)
    {
        ...
        {
            printf(
                "TopK (%d) is larger than max supported number (%d). "
    ...
    cudaMemcpy(
        mSkipDecodeHost.data(), mSkipDecodeDevice.data(), mSkipDecodeDevice.getSizeInBytes(), cudaMemcpyDeviceToHost);
    std::vector<std::int32_t> runtimeTopKs(mDecoderDomain.getBatchSize());
    cudaMemcpy(
        runtimeTopKs.data(), mRuntimeTopKDevice.data(), mRuntimeTopKDevice.getSizeInBytes(), cudaMemcpyDeviceToHost);
    {
    ...
    std::shared_ptr<BaseDecodingOutputs> const& outputs, std::shared_ptr<BaseDecodingInputs> const& baseInputs)
{
    ...
    auto batchSlotsHost = inputs->batchSlots ? tensorCastOrNull<int>(inputs->batchSlots->get()) : batchSlotsVec.data();
    ...
    FinishedState const* finishedInput = (inputs->finished) ? reinterpret_cast<FinishedState const*>(
                                             tensorCastOrNull<FinishedState::UnderlyingType>(inputs->finished->get()))
                                                            : nullptr;
    FinishedState* finishedOutput = (outputs->finished)
        ? reinterpret_cast<FinishedState*>(tensorCastOrNull<FinishedState::UnderlyingType>(outputs->finished->get()))
        : nullptr;
    ...
    params.outputIdsPtrs = tensorCastOrNull<std::int64_t*>(outputs->outputIdsPtr);
    ...
    params.sequenceLengths = tensorCastOrNull<std::int32_t>(outputs->sequenceLength);
