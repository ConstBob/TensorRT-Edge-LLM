...
inline size_t calcAlignedSize(std::vector<size_t> const& sizes, const size_t ALIGN_BYTES = 256)
{
...
inline void calcAlignedPointers(
...
public:
    ...
    TensorWrapper(void* data, std::vector<std::int64_t> const& dim, DataType dtype)
        : _data(data)
        , _dim(dim)
        , _dtype(dtype)
    {
    ...
    {
        auto size = std::accumulate(_dim.begin(), _dim.end(), 1, std::multiplies<size_t>());
...
using TensorConstPtr = std::shared_ptr<TensorWrapper const>;

template <typename T>
T const* tensorCast(TensorWrapper const& tensor)
{
    if (TRTDataType<typename std::remove_cv<T>::type>::value != tensor.getDataType())
    {
        throw std::bad_cast();
    }
...
template <typename T>
T* tensorCast(TensorWrapper& tensor)
{
    if (TRTDataType<typename std::remove_cv<T>::type>::value != tensor.getDataType())
    {
        throw std::bad_cast();
...
template <typename T>
T const* tensorCastOrNull(TensorWrapper const* tensor)
{
    if (tensor)
    {
...
template <typename T>
T* tensorCastOrNull(TensorWrapper* tensor)
{
    if (tensor)
    {
        return tensorCast<T>(*tensor);
...
template <typename T>
T* tensorCastOrNull(TensorPtr const& tensor)
{
    if (tensor)
    {
        return tensorCast<T>(*tensor);
...
template <typename T>
T const* tensorCastOrNull(TensorConstPtr const& tensor)
{
    if (tensor)
    {
        return tensorCast<T>(*tensor);
...
inline auto copyTensorWrapper(
    TensorWrapper const& src, TensorWrapper& dst, cudaMemcpyKind kind = cudaMemcpyDeviceToDevice)
{
