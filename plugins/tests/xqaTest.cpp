...
void runTest(int32_t batchSize, int32_t seqLen, bool testPerf, bool refCheck, bool verbose = false, bool saveData = false) {
    ...
    if (batchSize == 0) {
        uint32_t const ctaPerWave = (uint32_t)prop.multiProcessorCount * (useQGMMA && headElems * roundUp(headGrpSize * beamWidth, 8U) <= 128 * 16 ? 3 : 2);
    ...
    size_t const totalBytes = cacheBytes + inputBytes + outputBytes + seqLenListBytes + ctxLenListBytes + cacheIndirBytes;
    ...
    if (verbose) {
        ...
        printf("cacheBytes= %lu  qByte= %lu  outbytes= %lu  totalBytes= %lu\n", cacheElemSize*totalNbCacheElems, inputElemSize*qElems, inputElemSize*outElems, totalBytes);
        ...
        for (uint32_t req = 0; req < batchSize; req++) {
            for (uint32_t b = 0; b < beamWidth; b++) {
                for (uint32_t idxKHead = 0; idxKHead < nbKHeads; idxKHead++) {
                    auto const kv = reinterpret_cast<GMemCacheHead(*)[beamWidth][2][nbKHeads][maxSeqLen]>(cacheHeads.get());
                    ...
                    if (useQGMMA) {
                        refOutput = refFlashAttention<CacheElem, 64>(&qHeads[req][b][headGrpSize * idxKHead], kCacheSeq, vCacheSeq, seqLen, kvCacheScale[0], xScale);
                        // refOutput = refAttention<CacheElem>(&qHeads[req][b][headGrpSize * idxKHead], kCacheSeq, vCacheSeq, seqLen, kvCacheScale[0], xScale);
                    }
                    else {
                        // refOutput = refFlashAttention<InputElem, 64>(&qHeads[req][b][headGrpSize * idxKHead], kCacheSeq, vCacheSeq, seqLen, kvCacheScale[0], xScale);
                        refOutput = refAttention<InputElem>(&qHeads[req][b][headGrpSize * idxKHead], kCacheSeq, vCacheSeq, seqLen, kvCacheScale[0], xScale);
                    }
                    if (saveData) {
