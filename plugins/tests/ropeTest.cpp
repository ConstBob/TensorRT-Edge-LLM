...
void runRopeTestContext(int32_t batchSize, int32_t sequenceLen)
{
    ...
    invokeContextApplyRopeUpdateKVFP16(qkv_device_ptr, nullptr, kvcache_ptr, seqlen_device_ptr,
        nbQHeads, nbKHeads, sizePerHead, kvcacheCapacity, kROPE_BASE_FREQUENCY, kROPE_SCALE, sequenceLen, stream);
...

    {
        ...
        checkCuda(cudaMemcpy(temp_qkvUpdatedData.data(), qkv_device_ptr, sizeof(half) * totalQKVElems, cudaMemcpyDeviceToHost));
        checkCuda(cudaMemcpy(temp_kvCacheHost.data(), kvcache_ptr, sizeof(half) * totalKVCacheElems, cudaMemcpyDeviceToHost));

        std::transform(temp_kvCacheHost.begin(), temp_kvCacheHost.end(), kvCacheHost.begin(), [](half value){
            return __half2float(value);
        });
        std::transform(temp_qkvUpdatedData.begin(), temp_qkvUpdatedData.end(), qkvUpdatedData.begin(), [](half value){
            return __half2float(value);
        });
    }
            ...
            for (int32_t h = 0; h < nbQHeads; ++h)
            {
                for (int32_t i = 0; i < sizePerHead / 2; ++i)
                {
                    if (h % 4 == 0)
                    {
                        ...
                        {
                            printf("At %d %d %d %d. The value of dst : %f %f .\n", b, s, h, i, kDstPair.x, kDstPair.y);
                            printf("At %d %d %d %d. The value of kvcache : %f %f .\n", b, s, h, i, kKvPair.x, kKvPair.y);
                            check(checkDataPair(vSrcPair, vKvPair, 1e-8), "Check v of kcache");
                        }
                        // --------------------------------------check k completes ------------------------------------
                    }
...
void runRopeTestGeneration(int32_t batchSize, int32_t sequenceLen)
{
    ...
    for (int b = 0; b < batchSize; ++b)
    {
        for (int h = 0; h < (nbKHeads + nbVHeads); ++h)
        {
            for (int s = 0; s < sequenceLen - 1; ++s)
            {
                ...
                {
                    int offset = b * (nbKHeads + nbVHeads) * kvcacheCapacity * sizePerHead + h * kvcacheCapacity * sizePerHead +
                        s * sizePerHead + i;
    ...
    invokeGenerationApplyRopeUpdateKVFP16(qkv_device_ptr, q_ptr, kvcache_ptr, seqlen_device_ptr,
        nbQHeads, nbKHeads, sizePerHead, kvcacheCapacity, kROPE_BASE_FREQUENCY, kROPE_SCALE, 1, stream);
    ...
    std::vector<float> qUpdatedData(totalQElems);
    {
        ...
        std::transform(temp_qUpdatedData.begin(), temp_qUpdatedData.end(), qUpdatedData.begin(), [](half value){
            return __half2float(value);
        });
    }
    ...
    for (int b = 0; b < batchSize; ++b)
    {
        for (int h_kv = 0; h_kv < nbKHeads; ++h_kv)
        {
            for (int i = 0; i < sizePerHead / 2; ++i)
            {
                ...
                int srcOffsetV = b * (nbQHeads + nbKHeads + nbVHeads) * sizePerHead + (nbQHeads + nbKHeads) * sizePerHead 
                    + h_kv * sizePerHead + i * 2;
                int kvCacheOffsetV = b * (nbKHeads + nbVHeads) * kvcacheCapacity * sizePerHead + nbKHeads * kvcacheCapacity * sizePerHead
