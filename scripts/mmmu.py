"""
Adapted from https://github.com/MMMU-Benchmark/MMMU/blob/main/mmmu
"""
import math
import json
...
def get_multi_choice_info(result):
    """
    Given options A~I for multiple choice question
...
def evaluate_mmmu(args):
    ...
    for i in range(len(df)):
        ...
        output_dict_w_cat[category].update({
            data_id: output
        })
    ...
    for category in CAT_SHORT2LONG.values():
        ...
        examples_to_eval = []
        for data_id, result in cat_outputs.items():
            pred = result["pred"]
            parsed_pred = ""
            if isinstance(pred, str):
                if result["question_type"] == "multiple-choice":
                    index2ans, all_choices = get_multi_choice_info(result)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str, 
        required=True,
        help="The path to mmmu output csv."
    )
    parser.add_argument(
        "--output_path", 
        type=str, 
        required=True,
        help="If set, dump the evaluation result to output file."
    )
