...
You will first need to export the model from PyTorch to ONNX. We also use `onnx_graphsurgeon` to convert the Attention module into a TensorRT Plugin in the same script.

Please see the export folder [README.MD](./export/README.md)
...
Please notice that the `maxSeqLen` must be identical to kvCache capacity of the attention plugin.
