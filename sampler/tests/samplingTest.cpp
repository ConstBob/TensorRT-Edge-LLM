...
int main()
{
    ...
    cudaMemcpy(logitsDevice, halfLogit.data(), sizeof(half) * logit.size(), cudaMemcpyHostToDevice);

    auto inputs = std::make_shared<SamplingInputs>(batchSize);
    inputs->endIds
        = std::make_shared<TensorWrapper>(endIds, std::vector<int64_t>{maxBatchSize}, TRTDataType<int32_t>::value);
    inputs->logits = std::make_shared<TensorWrapper const>(
        logitsDevice, std::vector<int64_t>{batchSize, vocabSize}, TRTDataType<half>::value);
    ...
    inputs->finished = std::make_shared<TensorWrapper>(
        finished, std::vector<int64_t>{maxBatchSize}, TRTDataType<FinishedState::UnderlyingType>::value);

    auto outputs = std::make_shared<BaseDecodingOutputs>(std::make_shared<TensorWrapper>(
        outputIds, std::vector<int64_t>{maxBatchSize, maxSeqlen}, TRTDataType<int64_t>::value));
    outputs->outputIdsPtr = std::make_shared<TensorWrapper>(
        outputIdsPtr, std::vector<int64_t>{maxBatchSize}, TRTDataType<int32_t*>::value);
    outputs->sequenceLength
        = std::make_shared<TensorWrapper>(seqlen, std::vector<int64_t>{maxBatchSize}, TRTDataType<int32_t>::value);
    outputs->finished = std::make_shared<TensorWrapper>(
        finished, std::vector<int64_t>{maxBatchSize}, TRTDataType<FinishedState::UnderlyingType>::value);

    for (int step = 0; step < 5; step++)
    {
        cudaMemcpy(logitsDevice, halfLogit.data() + step * vocabSize, sizeof(half) * vocabSize, cudaMemcpyHostToDevice);

        topkLayer.forwardAsync(outputs, inputs);
        sync_check_cuda_error();

        cudaMemcpy(outputIdsHost, outputIds, sizeof(outputIdsHost), cudaMemcpyDeviceToHost);
