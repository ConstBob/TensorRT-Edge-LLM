...
# Only show public members by default
breathe_default_members = ('members', )
breathe_domain_by_extension = {
    "h": "cpp",
    "cuh": "cpp",
}
# Show only documented members (this will hide undocumented private members)
breathe_show_define_initializer = True
breathe_show_enumvalue_initializer = True
# Don't include undocumented members
...
def get_all_header_files():
    """
    Get a flat list of all header files for breathe_projects_source configuration.
    """
    module_headers = get_cpp_directory_structure()
    all_headers = []
    for headers in module_headers.values():
...
def generate_module_rst_files():
    ...
    for module_name, headers in module_headers.items():
        ...
        # Store module data for main index
        modules_data[module_name] = rst_files

    # Update main index.rst with all header files organized by module
    update_index_rst(source_dir, modules_data)

    return list(modules_data.keys())


def update_index_rst(source_dir, modules_data):
    """
    Update index.rst to link to module index pages, creating a hierarchical structure.
    The main index lists module index pages, which in turn list individual API files.
    ...
    index_content = """.. TensorRT Edge-LLM documentation master file, created by
...
for large language models on edge devices.
...
"""
    ...
    for module_name in sorted(modules_data.keys()):
        index_content += f"   {CPP_API_DIR}/{module_name}/index\n"
...
"""
    ...
    print(
        f"Updated {index_file} with {len(modules_data)} modules linking to their index pages"
    )


# Populate breathe_projects_source with all header files for autodoxygenfile
all_headers = get_all_header_files()
breathe_projects_source = {"TensorRT-Edge-LLM": ("../../cpp", all_headers)}

# Generate RST files at configuration time
