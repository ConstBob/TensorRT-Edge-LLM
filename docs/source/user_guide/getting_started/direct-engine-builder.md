...
configuration and safetensors directly, selects model-owned Python component
definitions, and writes the same engine and runtime-artifact layout consumed by
...
`-DENABLE_CUTE_DSL=ALL` includes optional kernels needed by model families such
as Qwen3.5 and Gemma4. A smaller plugin build is valid when it still contains
every operation selected by the checkpoint.
...
do not require a precision switch.
...
Runtime configs, tokenizers, chat templates, embeddings, preprocessing files,
and externalized quantized weights are written beside the component engines.
...
EAGLE3, DFlash, and Gemma4 MTP use paired checkpoints:
...
Replace `eagle3` with `dflash` or `gemma4_mtp` as appropriate. Qwen3.5 native
MTP reads draft layers from the target checkpoint:
...
Run the resulting directory with the normal `llm_inference --specDecode`
options.
...
Qwen2/2.5/3-VL, Qwen3.5 dense and MoE, Qwen3-ASR, Qwen3-Omni, Qwen3-TTS,
InternVL3/3.5, Phi-4 Multimodal, Nemotron-H/Omni, Gemma4, and Alpamayo.
Unsupported `model_type` values fail before TensorRT network creation and list
...
| FP8 dense LLM | Implemented | Qwen2.5 on RTX 5090 |
...
| EAGLE3 | Implemented | Qwen3 on A30 |
| Qwen3.5 MTP, DFlash, and Gemma4 MTP | Implemented | Not yet |
| Visual, audio, TTS, omni, and action components | Implemented for registered families | Not yet |
...
- DiffusionGemma and dSpark model definitions are not registered.
- Qwen3-Omni-Next model definitions are not registered.
- Qwen3-TTS voice-cloning encoder components are not registered.
- Gemma4 unified visual/audio checkpoint variants are not registered.
