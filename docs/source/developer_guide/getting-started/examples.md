...
## Example 6: ASR (Automatic Speech Recognition) Inference
...
**Success!** 🎉 Check `output_asr.json` for the speech recognition transcription.

---

## Example 7: MoE (Mixture of Experts) — Qwen3-30B-A3B-GPTQ-Int4

Complete workflow for Mixture of Experts (MoE) models using a pre-quantized GPTQ-Int4 model. We only support **CPU-only export** for this model, so you can run the export step on a machine without a GPU.

**Model:** [Qwen3-30B-A3B-GPTQ-Int4](https://huggingface.co/Qwen/Qwen3-30B-A3B-GPTQ-Int4)

**Prerequisites:** Install `gptqmodel` (CPU-only) and `optimum` 2.0.0:
...
**Success!** 🎉 MoE export uses CPU-only; build and inference use the same `llm_build` and `llm_inference` as standard LLMs.

---

## Example 8: Qwen3-TTS Text-to-Speech

For the complete Qwen3-TTS workflow including dependency installation, export, engine build, and inference, see the dedicated guide:

**[Qwen3-TTS Quick Start Guide](tts-quick-start.md)**
