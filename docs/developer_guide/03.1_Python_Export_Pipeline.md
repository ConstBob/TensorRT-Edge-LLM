...
tensorrt-edgellm-quantize-llm \
  ...
  --quantization fp8 \
  --output_path quantized/qwen2.5-0.5b-fp8

# Step 2: Export to ONNX
tensorrt-edgellm-export-llm \
  --model quantized/qwen2.5-0.5b-fp8 \
  --export_path onnx_models/qwen2.5-0.5b
```
...
tensorrt-edgellm-export-llm \
  ...
  --export_path onnx_models/qwen2.5-vl-3b \
  --vlm

# Export visual encoder
tensorrt-edgellm-export-visual \
  --model Qwen/Qwen2.5-VL-3B-Instruct \
  --export_path onnx_models/qwen2.5-vl-3b/visual_enc_onnx_qwen2
```
...
tensorrt-edgellm-quantize-llm \
  --model Qwen/Qwen2.5-0.5B-Instruct \
  --quantization fp8 \
  --output_path quantized/qwen2.5-0.5b-base

# Export base model
tensorrt-edgellm-export-llm \
  --model quantized/qwen2.5-0.5b-base \
  --export_path onnx_models/qwen2.5-0.5b_eagle3_base

# Quantize draft model
tensorrt-edgellm-quantize-draft \
  --base_model quantized/qwen2.5-0.5b-base \
  --draft_model path/to/eagle_draft \
  --output_path quantized/qwen2.5-0.5b-draft

# Export draft model
tensorrt-edgellm-export-draft \
  --base_model quantized/qwen2.5-0.5b-base \
  --draft_model quantized/qwen2.5-0.5b-draft \
  --export_path onnx_models/qwen2.5-0.5b_eagle3_draft
```
...
# Insert LoRA support
tensorrt-edgellm-insert-lora \
  --onnx_path onnx_models/qwen2.5-0.5b/model.onnx \
  --output_path onnx_models/qwen2.5-0.5b/lora_model.onnx \
  --max_rank 64

# Process LoRA weights
tensorrt-edgellm-process-lora \
  --lora_weights path/to/lora_adapter \
  --output_path lora_weights/adapter.safetensors
```
...
tensorrt-edgellm-quantize-llm \
  ...
  --quantization fp8 \
  --device cpu \
  --output_path quantized/qwen2.5-7b-fp8
```
